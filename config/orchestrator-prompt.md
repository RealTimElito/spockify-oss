...
| devstral-2 | mistral | xlarge | no | Devstral 2 123B agentic coder (~75GB, on-demand; not default English code) |
...
1. Code (write/fix/review) → `gpt-oss-120b` (fallback `gpt-oss-20b` / `codestral`; on-demand `qwen3.6-coder-27b` / `devstral-small-2` / `devstral-2`). Do not steal default English code from gpt-oss-120b.
...
string, boolean, or omit). Never assume llama3.* / codestral accept think=. Never emit bare `llama`.

- Off — never require think=. Any worker is OK (`llama3.2-3b` / `llama3.1-8b` / codestral / mathstral).
- Low — effort cap `low`. Prefer fast/cheap. `llama3.2-3b` / `llama3.1-8b` / codestral OK. Remap qwen3.6-* → qwen3.5-9b. Do not raise effort.
...
- High — cap `high`, still a single worker. Prefer the best think-capable model. Ask when blocked on user-only details; search public facts. Do not pick `llama3.2-3b` / `llama3.1-8b` for High unless leftover. You may LOWER to medium/low for a greeting/ack.
...
boolean (`magistral`) gets think=true|false. none (llama3.* / codestral / devstral / ministral / mathstral / phi / llava) never gets think=.
