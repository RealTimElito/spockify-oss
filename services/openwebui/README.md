...
└── branding/               # Tracked Spockify favicon/logo (Docker/GHCR context)
```
...
Branding PNGs/ICO live in `branding/` (tracked) so GHCR and `docker compose build`
ship the real Spockify mark. Homelab `make build-openwebui` also syncs from
`k8s/openwebui/assets/` before the image build.

## Build approach

1. **Source:** vendored `upstream/` with Spockify edits already applied
2. **Frontend:** `npm ci && npm run build` inside Docker
3. **Branding:** tracked `branding/` PNGs/ICO copied into the frontend static tree
...
Server STT uses **local faster-whisper** (`AUDIO_STT_ENGINE` unset). On Spark/Spock the model is **`large-v3`** (int8, CPU) with **auto language detection** (`WHISPER_LANGUAGE` unset, `WHISPER_MULTILINGUAL=true`). Weights live on the OpenWebUI PVC under `cache/whisper/models`.
