...
async def _user_from_litellm_virtual_key(api_key: str):
    """Accept LiteLLM virtual keys (spockify.eu/ui, CLI device login) as OWUI users.

    IDE API-key sign-in stores a LiteLLM ``sk-…`` that never appears in the OWUI
    ``user.api_key`` column. Without this fallback, ``/api/v1/spockify/ghost/*``
    (and other Spockify proxies) return 401 while ``/v1/chat/completions`` still
    works — so Tab ghosts silently fail for the most common IDE auth path.
    """
    import hashlib
    import time
    ...
    from open_webui.utils.spockify_cli_device import _litellm_base

    base = _litellm_base()
    if not base or not api_key.startswith('sk-'):
        return None
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.get(
                ...
                if resp.status < 400:
                    ...
                    # Some virtual keys cannot read /key/info; /v1/models is enough.
                    async with session.get(
                        f'{base}/v1/models',
                        headers={'Authorization': f'Bearer {api_key}'},
                    ) as resp2:
                        if resp2.status >= 400:
                            return None
    except Exception as exc:  # noqa: BLE001 — auth fallback must fail soft
        log.debug('LiteLLM virtual key check failed: %s', exc)
        return None
    ...
    uid = info.get('user_id') or meta.get('user_id') or meta.get('owui_user_id')
    if uid:
        existing = await Users.get_user_by_id(str(uid))
        if existing is not None:
            return existing

    digest = hashlib.sha256(api_key.encode('utf-8')).hexdigest()[:16]
    now = int(time.time())
    email = str(meta.get('email') or f'litellm-{digest}@spockify.local')
    name = str(
        meta.get('key_alias')
        or info.get('key_alias')
        or meta.get('name')
        or 'API key'
    )
    return UserModel(
...
async def get_current_user_by_api_key(request, api_key: str):
    ...
    litellm_virtual = False

    if user is None:
        user = await _user_from_litellm_virtual_key(api_key)
        litellm_virtual = user is not None
        if user is None:
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
                detail=ERROR_MESSAGES.INVALID_TOKEN,
    ...
    # LiteLLM product keys are not gated by OWUI's enable_api_keys toggle.
    if not litellm_virtual and (
        not request.state.enable_api_keys
        or (
            user.role != 'admin'
    ...
    if (
        not litellm_virtual
        and request.app.state.config.ENABLE_API_KEYS_ENDPOINT_RESTRICTIONS
    ):
        allowed_paths = [
    ...
    if ENABLE_OTEL:
        ...
        if current_span:
            ...
            current_span.set_attribute(
                'client.auth.type',
                'litellm_virtual_key' if litellm_virtual else 'api_key',
            )

    # Synthetic litellm:* users are not in the OWUI user table.
    if not str(user.id).startswith('litellm:'):
        await Users.update_last_active_by_id(user.id)
    return user


# Verified roles include Wave 9 family/guest (capped) — not pending.
VERIFIED_USER_ROLES = frozenset({'user', 'admin', 'family', 'guest'})
