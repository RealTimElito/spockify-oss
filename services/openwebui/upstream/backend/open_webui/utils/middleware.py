...
from open_webui.utils.video_options import resolve_video_length
from open_webui.utils.videos.comfyui_video import (
    i2v_motion_prompt,
    video_generation_configured,
    video_generations,
    video_i2v_configured,
)
...
IMAGE_PRE_GENERATED_ACK = "Here's the image you requested!"
IMAGE_AGENT_JSON_FALLBACK = (
    "I couldn't display that image from the model reply. "
    "Try “show me a photo of …” or “draw me …” — a real image should appear."
)

VIDEO_PRE_GENERATED_ACK = "Here's the video you requested!"


def _extract_json_object(text: str) -> dict | None:
    """Return a parsed JSON object from text, or None."""
    stripped = text.strip()
...
def looks_like_agent_action_json(text: str) -> bool:
    """Detect ReAct/LangChain agent JSON (e.g. dalle.text2im) in plain model output."""
    obj = _extract_json_object(text)
    if obj and 'action' in obj:
        action = str(obj.get('action', '')).lower()
        if is_image_tool_action(action):
            return True
        if 'action_input' in obj or 'thought' in obj:
            return True
    # Malformed JSON still must never reach the user.
    return text_looks_like_image_agent_json(text)


def sanitize_agent_action_json_text(text: str) -> str:
    """Strip agent tool-call JSON blobs from assistant text."""
    if not text or not looks_like_agent_action_json(text):
        return text
    return ''


def _image_pre_generated_ack(metadata: dict | None = None, form_data: dict | None = None) -> str:
    if form_data and form_data.get('__image_pre_generated_ack__'):
        return str(form_data['__image_pre_generated_ack__'])
    if metadata and metadata.get('__image_pre_generated_ack__'):
        return str(metadata['__image_pre_generated_ack__'])
    return IMAGE_PRE_GENERATED_ACK


def finalize_image_gen_assistant_text(text: str, metadata: dict) -> str:
    """Replace leaked agent/tool JSON with a friendly message (never show raw dalle JSON)."""
    if not text:
        return text
    stripped = text.strip()
    ack = _image_pre_generated_ack(metadata)
    if looks_like_agent_action_json(stripped):
        if metadata.get('__image_pre_generated__'):
            return ack
        return IMAGE_AGENT_JSON_FALLBACK
    if metadata.get('__image_pre_generated__') and not stripped:
        return ack
    return text


def sanitize_image_gen_output(output: list, metadata: dict) -> list:
    """Strip leaked dalle/ReAct JSON from OR output (always; ack when pre-generated)."""
    for item in output:
        if item.get('type') != 'message':
            continue
        for part in item.get('content', []):
            if part.get('type') == 'output_text' and 'text' in part:
                part['text'] = finalize_image_gen_assistant_text(
                    part.get('text', ''), metadata
                )
    return output


def build_pre_generated_image_chat_response(form_data: dict):
    """Synthetic chat completion when middleware already created the image (skip worker LLM)."""
    ack = _image_pre_generated_ack(form_data=form_data)
    ...
    async def get_content_from_response(response) -> Optional[str]:
        ...
        if hasattr(response, 'body_iterator'):
            async for chunk in response.body_iterator:
                line = chunk.decode('utf-8', 'replace') if isinstance(chunk, bytes) else str(chunk)
...
async def chat_image_generation_handler(request: Request, form_data: dict, extra_params: dict, user):
    ...
    async def emit_image_status(description: str, done: bool = False):
    ...
    if chat_id.startswith('local:') or chat_id.startswith('channel:'):
        ...
        chat = await Chats.get_chat_by_id_and_user_id(chat_id, user.id)
        await emit_image_status('Preparing GPU…', done=False)

        # GPU unload also runs inside image_generations for API/tool paths;
        # do it here so the status line updates before the long Flux wait.
    ...
    if len(input_images) > 0 and request.app.state.config.ENABLE_IMAGE_EDIT:
        ...
        try:
            ...
            await emit_image_status('Image created', done=True)
            ...
            metadata['__image_pre_generated__'] = True
        except Exception as e:
            log.debug(e)
        ...
        if request.app.state.config.ENABLE_IMAGE_PROMPT_GENERATION:
            try:
                await emit_image_status('Preparing image prompt…', done=False)
        ...
        try:
            await emit_image_status('Generating image…', done=False)
            # Q4: optional aspect/style chips from the composer (features).
            img_features = extra_params.get('__features__') or {}
            aspect = img_features.get('spockify_image_aspect')
            style = img_features.get('spockify_image_style')
            sized_prompt = apply_style_to_prompt(prompt, style)
            create_kwargs: dict = {'prompt': sized_prompt}
            if aspect:
                create_kwargs['size'] = resolve_image_size(aspect)
            images = await image_generations(
                request=request,
                form_data=CreateImageForm(**create_kwargs),
                metadata={
                    'chat_id': metadata.get('chat_id', None),
                    'message_id': metadata.get('message_id', None),
                },
                user=user,
                on_progress=emit_image_status,
                # Already unloaded above when we have a real chat_id.
                prepare_gpu=bool(chat_id.startswith('local:') or chat_id.startswith('channel:')),
            )
...
def _mark_pre_generated_ack(form_data: dict, metadata: dict, ack: str) -> None:
    metadata['__image_pre_generated__'] = True
    metadata['__image_pre_generated_ack__'] = ack
    form_data['__image_pre_generated_ack__'] = ack


async def chat_web_image_handler(request: Request, form_data: dict, extra_params: dict, user):
    """Search the web for a photo, store it, and embed it inline (skip LLM)."""
    metadata = extra_params.get('__metadata__', {})
    chat_id = metadata.get('chat_id', None)
    __event_emitter__ = extra_params.get('__event_emitter__', None)
    ...
    user_message = get_last_user_message(messages)
    query = resolve_web_image_query(user_message, messages)
    if not query:
        await emit_status('Could not find a displayable photo', done=True)
        _mark_pre_generated_ack(
            form_data,
            metadata,
            'I need a clearer subject — try “show me a photo of …”.',
        )
        form_data['messages'] = add_or_update_system_message(
            '<context>The user asked for another photo but there is no clear subject '
            'from this chat. Ask what they want to see (e.g. “show me a photo of …”). '
            'Do not invent DALL·E / tool JSON, and do not claim you cannot show images.</context>',
            form_data['messages'],
        )
        return form_data

    await emit_status(f'Searching for a photo of {query}…', done=False)

    try:
        stored_url, source_url = await fetch_embeddable_web_image(
            request,
            query,
            metadata={
                'chat_id': metadata.get('chat_id'),
            ...
            user=user,
            searxng_query_url=getattr(request.app.state.config, 'SEARXNG_QUERY_URL', None),
            messages=messages,
        )
    except Exception as exc:
        log.warning('web image embed failed: %s', exc)
        stored_url, source_url = None, None
    ...
    if not stored_url:
        ...
        _mark_pre_generated_ack(
            form_data,
            metadata,
            "I couldn't find a displayable photo for that. Try a more specific subject "
            'or say “draw me …” to generate one.',
        )
        form_data['messages'] = add_or_update_system_message(
            '<context>A web photo was requested but could not be fetched or displayed. '
            'Apologize briefly and suggest the user try a more specific subject, or ask '
            'to generate an image with "draw me" / "generate an image of". Do not claim '
            'you cannot display images in general, and do not paste stock-photo site '
            'links as a substitute. Do not output DALL·E or tool JSON.</context>',
            form_data['messages'],
        )
        return form_data

    await emit_status('Photo ready', done=True)
    await __event_emitter__(
        {
            'type': 'files',
            'data': {
    ...
    ack = web_image_assistant_ack(query, stored_url)
    metadata['__image_pre_generated__'] = True
    metadata['__web_image_embedded__'] = True
    metadata['__image_pre_generated_ack__'] = ack
    if source_url:
        metadata['__web_image_source__'] = source_url
    form_data['__image_pre_generated_ack__'] = ack
    log.info('Embedded web image for query %r from %s', query, source_url)
    return form_data


async def chat_video_generation_handler(request: Request, form_data: dict, extra_params: dict, user):
    """Generate a short T2V/I2V clip via ComfyUI/LTX-Video, skip the LLM.

    Mirrors chat_image_generation_handler: pre-LLM intent detection already
    ran in middleware(), this does the GPU-prep / generate / upload /
    synthetic-ack dance for video. With a chat image attachment, uploads to
    ComfyUI and runs the I2V workflow; otherwise text-to-video.
    """
    metadata = extra_params.get('__metadata__', {})
    chat_id = metadata.get('chat_id', None)
    __event_emitter__ = extra_params.get('__event_emitter__', None)
    ...
    async def emit_video_status(description: str, done: bool = False):
        await __event_emitter__(
            {
                'type': 'status',
                'data': {
    ...
    if not video_generation_configured():
        await emit_video_status('Video generation is not configured on this server', done=True)
        form_data['messages'] = add_or_update_system_message(
            '<context>The user asked for a generated video but video generation is not '
            'configured on this server. Apologize briefly and do not invent a video or '
            'claim one was created.</context>',
            form_data['messages'],
        )
        return form_data
    ...
    if chat_id.startswith('local:') or chat_id.startswith('channel:'):
        ...
        chat = await Chats.get_chat_by_id_and_user_id(chat_id, user.id)
        await emit_video_status('Preparing GPU…', done=False)
    ...
    user_message = get_last_user_message(message_list)
    extracted = extract_video_prompt(user_message) or user_message

    # Most recent attached image → I2V; otherwise T2V.
    input_images = []
    for idx, images in enumerate(get_images_from_messages(message_list)):
        if idx >= 1:
            break
        input_images.extend(images)
    source_image = input_images[0] if input_images else None

    if source_image and not video_i2v_configured():
        await emit_video_status(
            'Image-to-video is not configured on this server', done=True
        )
        form_data['messages'] = add_or_update_system_message(
            '<context>The user attached an image and asked for a video, but '
            'image-to-video is not configured on this server. Apologize briefly '
            'and do not invent a video.</context>',
            form_data['messages'],
        )
        return form_data

    prompt = (
        i2v_motion_prompt(extracted, user_message) if source_image else extracted
    )

    # Q4-style: optional duration chip from the composer (features).
    vid_features = extra_params.get('__features__') or {}
    frame_count = resolve_video_length(
        vid_features.get('spockify_video_duration'),
        fallback=VIDEO_LENGTH,
    )

    await emit_video_status('Preparing video prompt…', done=False)

    try:
        await emit_video_status(
            'Generating video… (this can take a few minutes on shared GPU hardware)',
            done=False,
        )
        videos = await video_generations(
            request=request,
            prompt=prompt,
            # Prefer the full user message so spoken_narration can pick up
            # "say …" / "with narration:" even when LTX uses a cleaned subject.
            narration=user_message or extracted or prompt,
            metadata={
                'chat_id': metadata.get('chat_id', None),
                'message_id': metadata.get('message_id', None),
            },
            user=user,
            on_progress=emit_video_status,
            # Already unloaded above when we have a real chat_id.
            prepare_gpu=bool(chat_id.startswith('local:') or chat_id.startswith('channel:')),
            image=source_image,
            length=frame_count,
        ...
        await emit_video_status('Video created', done=True)
        ...
        system_message_content = (
            '<context>The requested video has been created by the system successfully and is '
            'now being shown to the user, playing inline in the chat. Let the user know that '
            'the video they requested has been generated.</context>'
        )
        _mark_pre_generated_ack(form_data, metadata, VIDEO_PRE_GENERATED_ACK)
    except Exception as e:
        log.debug(e)
        ...
        await emit_video_status(f'Video generation failed: {error_message}', done=True)

        system_message_content = (
            f'<context>Video generation was attempted but failed because of an error. The '
            f'system is currently unable to generate the video. Tell the user that the '
            f'following error occurred: {error_message}</context>'
        )
...
async def process_chat_payload(request, form_data, user, metadata, model):
    ...
    if folder_id and user:
        ...
        if folder and folder.data:
            # Spockify W4.1: inject short project summary for chats in this folder.
            project_summary = (folder.data.get('project_summary') or '').strip()
            if project_summary:
                summary = project_summary[:2000]
                project_block = (
                    f'Project: {folder.name}\n'
                    f'Summary: {summary}\n'
                    'Use this project context when relevant; do not invent facts beyond it.'
                )
                form_data = await apply_system_prompt_to_body(
                    project_block, form_data, metadata, user
                )
    ...
    # Wave 6.1: optional sticky session note on the chat itself
    if chat_id and user:
        try:
            chat_row = await Chats.get_chat_by_id_and_user_id(chat_id, user.id)
            note = ''
            if chat_row and chat_row.meta:
                note = (chat_row.meta.get('spockify_session_note') or '').strip()
            if note:
                form_data = await apply_system_prompt_to_body(
                    f'Session note:\n{note[:2000]}',
                    form_data,
    ...
    if spockify_routed:
        ...
        if mode not in ('auto', 'on', 'off'):
            mode = 'auto'
        metadata['spockify_search_mode'] = mode
        marker = f'[spockify_search_mode:{mode}]'
        messages = form_data.get('messages') or []
        marker_only = re.compile(
            r'^\s*\[spockify_search_mode:(auto|on|off)\]\s*$', re.IGNORECASE
        )
        marker_any = re.compile(
            r'\[spockify_search_mode:(auto|on|off)\]', re.IGNORECASE
        )
        cleaned = []
        for msg in messages:
            ...
            if msg.get('role') == 'system' and isinstance(content, str) and marker_only.match(
                content
            ):
                continue
            if isinstance(content, str) and marker_any.search(content):
                msg = {**msg, 'content': marker_any.sub('', content).strip()}
                if msg.get('role') == 'system' and not msg['content']:
                    continue
            cleaned.append(msg)
        cleaned.insert(0, {'role': 'system', 'content': marker})
        form_data['messages'] = cleaned
        extra_headers = form_data.get('extra_headers')
        if not isinstance(extra_headers, dict):
            extra_headers = {}
        extra_headers['X-Spockify-Search-Mode'] = mode
        form_data['extra_headers'] = extra_headers

        # Call / features.voice → router prefers VOICE_CHAT_WORKER (still tools/web).
        if features.get('voice'):
            metadata['spockify_voice'] = True
            voice_marker = '[spockify_voice:1]'
            voice_only = re.compile(
                r'^\s*\[spockify_voice:(1|true|yes|on)\]\s*$', re.IGNORECASE
            )
            voice_any = re.compile(
                r'\[spockify_voice:(1|true|yes|on)\]', re.IGNORECASE
            )
            voice_cleaned = []
            for msg in form_data.get('messages') or []:
                if not isinstance(msg, dict):
                    voice_cleaned.append(msg)
                    continue
                content = msg.get('content')
                if msg.get('role') == 'system' and isinstance(content, str) and voice_only.match(
                    content
                ):
                    continue
                if isinstance(content, str) and voice_any.search(content):
                    msg = {**msg, 'content': voice_any.sub('', content).strip()}
                    if msg.get('role') == 'system' and not msg['content']:
                        continue
                voice_cleaned.append(msg)
            voice_cleaned.insert(0, {'role': 'system', 'content': voice_marker})
            form_data['messages'] = voice_cleaned
            extra_headers = form_data.get('extra_headers')
            if not isinstance(extra_headers, dict):
                extra_headers = {}
    ...
    if pack_ids:
        ...
        if not isinstance(extra_headers, dict):
            extra_headers = {}
        if isinstance(pack_ids, (list, tuple, set)):
            extra_headers['X-Spockify-Skills'] = ','.join(str(x) for x in pack_ids if x)
        elif isinstance(pack_ids, str) and pack_ids.strip():
            extra_headers['X-Spockify-Skills'] = pack_ids.strip()
        form_data['extra_headers'] = extra_headers
        metadata['spockify_skills'] = extra_headers.get('X-Spockify-Skills')

    # Wave 9.8 — forward user role for family/guest caps.
    if user is not None:
        extra_headers = form_data.get('extra_headers')
        if not isinstance(extra_headers, dict):
            extra_headers = {}
    ...
    # Auto-embed a real web photo for "show me a picture of …" (SearXNG →
    # download → OWUI file URL). Skip the LLM so it cannot claim it cannot
    # display images or dump Pexels/Unsplash links / DALL·E JSON.
    searx_configured = bool(
        getattr(request.app.state.config, 'SEARXNG_QUERY_URL', None)
    )
    if (
        user_message_requests_web_image(user_message)
        and not metadata.get('__image_pre_generated__')
    ):
        metadata['__web_image_auto__'] = True
        if searx_configured:
            log.info('Auto web-image embed from user prompt intent')
            form_data = await chat_web_image_handler(
                request, form_data, extra_params, user
            )
        else:
            log.warning('Web photo requested but SEARXNG_QUERY_URL is not configured')
            _mark_pre_generated_ack(
                form_data,
                metadata,
                'Photo search is not configured on this server. Try “draw me …” to generate an image.',
            )

    # Duration chips + video toggle live on features — set before any
    # ComfyUI video/image handler so spockify_video_duration is visible.
    extra_params['__features__'] = features

    # Auto-run ComfyUI/LTX when the Video toggle is on or the prompt matches
    # video intent (explicit video/clip/animation noun — see video_intent.py).
    video_toggle = isinstance(features, dict) and bool(features.get('video_generation'))
    if (
        (video_toggle or user_message_requests_video_generation(user_message))
        and not metadata.get('__image_pre_generated__')
    ):
        if video_generation_configured():
            log.info(
                'Triggered video generation (%s)',
                'toggle' if video_toggle else 'prompt intent',
            )
            metadata['__video_gen_auto__'] = True
            form_data = await chat_video_generation_handler(
                request, form_data, extra_params, user
            )
        else:
            log.warning('Video generation requested but COMFYUI_VIDEO_* env is not configured')
            _mark_pre_generated_ack(
                form_data,
                metadata,
                'Video generation is not configured on this server yet.',
            )
            form_data['messages'] = add_or_update_system_message(
                '<context>The user asked for a generated video but video generation is not '
                'configured on this server. Apologize briefly and do not invent a video or '
    ...
    # Auto-enable ComfyUI image pipeline for draw/generate prompts so users
    # do not need the obscure chat "Image Generation" toggle. Worker models
    # often emit raw dalle.text2im JSON instead of calling real tools.
    ...
    if (
        ...
        and not features.get('image_generation')
        and not metadata.get('__image_pre_generated__')
        and user_message_requests_image_generation(user_message)
    ):
        features['image_generation'] = True
        metadata['__image_gen_auto__'] = True
        log.info('Auto-enabled image_generation from user prompt intent')

    # Keep __features__ in sync after auto-enable mutations.
    extra_params['__features__'] = features
    if features:
        # Spockify Canvas: nudge only when Integrations → Canvas is on for this turn.
        if features.get('canvas'):
            form_data = await apply_system_prompt_to_body(
                (
                    'Spockify Canvas is enabled: for long documents, plans, or substantial '
                    'code, put the main deliverable in a fenced block tagged canvas '
                    '(markdown inside). Keep a short summary in the chat outside the fence. '
                    'HTML/SVG previews also open in Canvas. Skip the fence for short replies.'
                ),
                form_data,
                metadata,
                user,
        ...
        if (
            ...
            # Always run the pre-LLM ComfyUI path when the feature is on
            # (chat toggle or auto-intent). Native FC + router workers often
            # hallucinate dalle.text2im JSON instead of calling generate_image.
    ...
    if event_emitter:
        try:
            ...
            if 'selected_model_id' in response_data and not metadata.get('chat_id', '').startswith('channel:'):
                ...
                if response_data.get('worker'):
                    upsert_fields['spockifyWorker'] = response_data['worker']
                if 'web_search' in response_data:
                    upsert_fields['spockifyWebSearch'] = bool(
                        response_data['web_search']
                    )
                if response_data.get('routing_path'):
                    upsert_fields['spockifyRoutingPath'] = response_data[
                        'routing_path'
                    ]
                if response_data.get('reasoning'):
                    upsert_fields['spockifyReason'] = response_data['reasoning']
                if response_data.get('spockify_agents'):
                    upsert_fields['spockifyAgents'] = response_data[
                        'spockify_agents'
                    ]
                # Router search citations (OpenWebUI chip payload).
                citation_sources = response_data.get('sources') or []
                if citation_sources:
                    upsert_fields['sources'] = citation_sources
                await Chats.upsert_message_to_chat_by_id_and_message_id(
                    metadata['chat_id'],
                    metadata['message_id'],
                    upsert_fields,
            ...
            if choices and choices[0].get('message', {}).get('content'):
                content = finalize_image_gen_assistant_text(
...
async def streaming_chat_response_handler(response, ctx):
    ...
    if event_emitter:
        ...
        if is_spockify_router_model(model_id):
            ...
            if worker:
                path = hdrs.get(
                    'llm_provider-x-spockify-routing-path'
                ) or hdrs.get('x-spockify-routing-path')
                web_search = hdrs.get(
                    'llm_provider-x-spockify-web-search'
                ) or hdrs.get('x-spockify-web-search')
                reasoning = hdrs.get(
                    'llm_provider-x-spockify-reasoning'
                ) or hdrs.get('x-spockify-reasoning')
                if web_search == 'true':
                    desc = 'Searching the web…'
                elif worker.startswith('codestral') or worker.startswith('web-codestral'):
                    desc = 'Writing code…'
                elif worker.startswith('web-'):
                    desc = 'Looking that up…'
                else:
                    desc = 'Thinking…'
                latency = hdrs.get(
                    'llm_provider-x-spockify-latency-ms'
                ) or hdrs.get('x-spockify-latency-ms')
                cost = hdrs.get(
                    'llm_provider-x-spockify-cost-usd'
                ) or hdrs.get('x-spockify-cost-usd')
                hud_model = hdrs.get(
                    'llm_provider-x-spockify-hud-model'
                ) or hdrs.get('x-spockify-hud-model')
                prompt_tok = hdrs.get(
                    'llm_provider-x-spockify-tokens-prompt'
                ) or hdrs.get('x-spockify-tokens-prompt')
                comp_tok = hdrs.get(
                    'llm_provider-x-spockify-tokens-completion'
                ) or hdrs.get('x-spockify-tokens-completion')
                event_payload = {
                    'selected_model_id': 'spockify-auto',
                    'worker': worker,
                    'web_search': web_search == 'true',
                    'routing_path': path or '',
                ...
                if latency or cost or hud_model:
                    event_payload['spockify_hud'] = {
                        'worker': worker,
                        'model': hud_model or worker,
        ...
        async def response_handler(response, events):
            ...
            try:
                        ...
                        try:
                            ...
                            if data:
                                ...
                                if 'selected_model_id' in data:
                                    ...
                                    upsert_fields = {'selectedModelId': model_id}
                                    if data.get('worker'):
                                        upsert_fields['spockifyWorker'] = data['worker']
                                    if 'web_search' in data:
                                        upsert_fields['spockifyWebSearch'] = bool(
                                            data['web_search']
                                        )
                                    if data.get('routing_path'):
                                        upsert_fields['spockifyRoutingPath'] = data[
                                            'routing_path'
                                        ]
                                    if data.get('reasoning'):
                                        upsert_fields['spockifyReason'] = data[
                                            'reasoning'
                                        ]
                                    if data.get('spockify_agents'):
                                        upsert_fields['spockifyAgents'] = data[
                                            'spockify_agents'
                                        ]
                                    if data.get('spockify_hud'):
                                        upsert_fields['spockifyHud'] = data[
                                            'spockify_hud'
                                        ]
                                    if data.get('spockify_critique'):
                                        upsert_fields['spockifyCritique'] = data[
                                            'spockify_critique'
                                        ]
                                    # Router search citations → chips under the answer.
                                    citation_sources = data.get('sources') or []
                                    if citation_sources:
                                        upsert_fields['sources'] = citation_sources
                                    await Chats.upsert_message_to_chat_by_id_and_message_id(
                                        metadata['chat_id'],
                ...
                output = sanitize_image_gen_output(output, metadata)
