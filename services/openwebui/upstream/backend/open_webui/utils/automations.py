...
async def execute_automation(app, automation: AutomationModel) -> None:
    ...
    try:
        ...
        # Wave 6.2: optional project/folder for scheduled agents
        folder_id = None
        if isinstance(automation.meta, dict):
            folder_id = automation.meta.get('folder_id') or automation.meta.get('project_id')
        if folder_id:
            try:
                await Chats.update_chat_folder_id_by_id_and_user_id(
                    chat.id, automation.user_id, folder_id
                )
            except Exception as exc:
                log.warning('scheduled agent folder assign failed: %s', exc)
