...
from open_webui.utils.middleware import (
    ...
    build_pre_generated_image_chat_response,
...
async def lifespan(app: FastAPI):
    ...
    # Keep STT hot: load faster-whisper in the background so first Call turn
    # does not pay multi-second cold model init (does not block readiness).
    async def _warm_whisper():
        if not WHISPER_WARM_ON_STARTUP:
            return
        engine = (getattr(app.state.config, 'STT_ENGINE', None) or '').strip().lower()
        if engine and engine not in ('whisper', 'local'):
            return
        model_name = getattr(app.state.config, 'WHISPER_MODEL', None)
        if not model_name:
            return
        if app.state.faster_whisper_model is not None:
            return
        try:
            from open_webui.routers.audio import set_faster_whisper_model

            log.info('Warming faster-whisper model=%s ...', model_name)
            app.state.faster_whisper_model = await asyncio.to_thread(
                set_faster_whisper_model,
                model_name,
                WHISPER_MODEL_AUTO_UPDATE,
            )
            log.info('faster-whisper warm complete model=%s', model_name)
        except Exception as e:
            log.warning('faster-whisper warm failed: %s', e)
...
app.include_router(spockify.router, prefix='/api/v1/spockify', tags=['spockify'])
...
async def chat_completion(
    ...
    async def process_chat(request, form_data, user, metadata, model, tasks=None):
        try:
            ...
            if metadata.get('__image_pre_generated__'):
                response = build_pre_generated_image_chat_response(form_data)
            else:
    ...
    # Prefer the async task path whenever we have a chat + assistant message id.
    # session_id is optional — event_emitter broadcasts to the user room; requiring
    # socket id caused SSE to be returned to the UI's res.json() and surfaced as:
    # Unexpected token 'd', "data: {"id"... is not valid JSON
    if metadata.get('chat_id') and any(message_ids.values()):
        task_ids = []
        chat_id = metadata['chat_id']
            ...
            **(
                {
                    ...
                    # Env-gated LTX T2V/I2V (not PersistentConfig) — composer Video toggle.
...
async def get_manifest_json():
    if app.state.EXTERNAL_PWA_MANIFEST_URL:
        ...
        return {
            ...
            'description': f'{app.state.WEBUI_NAME} — local AI hub (chat, voice, knowledge, image).',
            'start_url': '/',
            'display': 'standalone',
