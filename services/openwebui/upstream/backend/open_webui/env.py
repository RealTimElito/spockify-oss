...
# Unload Ollama models before ComfyUI image gen on shared GPU (Spockify RTX 5090).
...
# ComfyUI text-to-video / image-to-video (LTX-Video / Lightricks). Env-based
# rather than admin-panel PersistentConfig, mirroring how IMAGE_GEN_* above is
# done — scoped intentionally simple since this is a fixed engine with two
# workflows (T2V + I2V), not a multi-provider setting like image generation.
ENABLE_VIDEO_GENERATION = os.getenv('ENABLE_VIDEO_GENERATION', 'False').lower() == 'true'
# Post-hoc edge-tts + ffmpeg mux onto silent LTX mp4 (utils/videos/video_audio.py).
# Default ON; set ENABLE_VIDEO_AUDIO=false to keep silent clips.
ENABLE_VIDEO_AUDIO = os.getenv('ENABLE_VIDEO_AUDIO', 'True').lower() == 'true'
COMFYUI_VIDEO_BASE_URL = os.getenv(
    'COMFYUI_VIDEO_BASE_URL', os.getenv('COMFYUI_BASE_URL', '')
).rstrip('/')
COMFYUI_VIDEO_API_KEY = os.getenv('COMFYUI_VIDEO_API_KEY', os.getenv('COMFYUI_API_KEY', ''))
# Raw workflow JSON text (same pattern as COMFYUI_WORKFLOW/COMFYUI_WORKFLOW_NODES
# in k8s/openwebui/deployment.yaml — populated directly from a ConfigMap key via
# configMapKeyRef, not a file path).
COMFYUI_VIDEO_WORKFLOW = os.getenv('COMFYUI_VIDEO_WORKFLOW', '')
COMFYUI_VIDEO_WORKFLOW_NODES = os.getenv('COMFYUI_VIDEO_WORKFLOW_NODES', '[]')
# Image-to-video (LoadImage → LTXVImgToVideo). Optional — when empty, chat
# attachments with video intent fall back to a clear "not configured" error
# instead of silently running text-to-video on the prompt alone.
COMFYUI_VIDEO_I2V_WORKFLOW = os.getenv('COMFYUI_VIDEO_I2V_WORKFLOW', '')
COMFYUI_VIDEO_I2V_WORKFLOW_NODES = os.getenv('COMFYUI_VIDEO_I2V_WORKFLOW_NODES', '[]')
VIDEO_GENERATION_MODEL = os.getenv('VIDEO_GENERATION_MODEL', 'ltxv-2b-0.9.8-distilled-fp8.safetensors')
...
# Generous — LTX-Video runs several minutes on Spark's shared GPU; the chat
