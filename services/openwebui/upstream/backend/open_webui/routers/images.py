...
async def image_generations(
    ...
    if prepare_gpu and request.app.state.config.IMAGE_GENERATION_ENGINE == 'comfyui':
        if on_progress:
            try:
    ...
    try:
        if request.app.state.config.IMAGE_GENERATION_ENGINE == 'openai':
            ...
            if res is None or not isinstance(res, dict) or not res.get('data'):
                raise ComfyUIUnavailableError(
                    'ComfyUI returned no image data (backend unreachable or workflow failed)'
                )
            log.debug(f'res: {res}')
        ...
        log.warning('ComfyUI image generation failed: %s', e)
        ...
        if isinstance(e, aiohttp.ClientResponseError):
            ...
            error = (
                f'Image backend unreachable ({request.app.state.config.IMAGE_GENERATION_ENGINE}): {e}'
            )
