...
async def get_headers_and_cookies(
    ...
    if metadata and metadata.get('spockify_search_mode'):
        headers['X-Spockify-Search-Mode'] = str(metadata['spockify_search_mode'])
    if metadata and metadata.get('spockify_skills'):
        headers['X-Spockify-Skills'] = str(metadata['spockify_skills'])
    if metadata and metadata.get('spockify_role'):
    ...
    if user is not None:
        ...
        if not headers.get('X-Spockify-Role'):
            headers['X-Spockify-Role'] = str(getattr(user, 'role', '') or '')
