...
from open_webui.utils.sanitize import sanitize_text_for_tts
...
# Microsoft Edge online neural voices (edge-tts). No API key required.
# Used when AUDIO_TTS_ENGINE=edge, or as fallback when engine is unset
# (browser SpeechSynthesis has no matching language voice).
#
# English defaults to Multilingual Neural (Ava) — newer conversational quality
# than classic Jenny. Swedish keeps Sofie (best free native sv voice).
...
_EDGE_CURATED_VOICES = {
    'sv-SE-SofieNeural': 'Sofie (Swedish)',
    'sv-SE-MattiasNeural': 'Mattias (Swedish)',
    'en-US-AvaMultilingualNeural': 'Ava Multilingual (US)',
    'en-US-EmmaMultilingualNeural': 'Emma Multilingual (US)',
    'en-US-AndrewMultilingualNeural': 'Andrew Multilingual (US)',
    'en-US-BrianMultilingualNeural': 'Brian Multilingual (US)',
    ...
    'de-DE-SeraphinaMultilingualNeural': 'Seraphina Multilingual (German)',
    'fr-FR-DeniseNeural': 'Denise (French)',
    'fr-FR-VivienneMultilingualNeural': 'Vivienne Multilingual (French)',
    'es-ES-ElviraNeural': 'Elvira (Spanish)',
    'nb-NO-PernilleNeural': 'Pernille (Norwegian)',
    'da-DK-ChristelNeural': 'Christel (Danish)',
}

# Neutral default for Call TTFA; override with AUDIO_TTS_EDGE_RATE (e.g. "-3%", "+5%").
_EDGE_DEFAULT_RATE = os.environ.get('AUDIO_TTS_EDGE_RATE', '+0%')
_EDGE_DEFAULT_PITCH = os.environ.get('AUDIO_TTS_EDGE_PITCH', '+0Hz')
_EDGE_DEFAULT_VOLUME = os.environ.get('AUDIO_TTS_EDGE_VOLUME', '+0%')


_SV_HINT = re.compile(
    r'\b(?:och|att|det|är|som|för|med|på|inte|ska|hej|tack)\b',
    re.IGNORECASE,
)


def _resolve_edge_voice(voice: Optional[str], text: str) -> str:
    """Pick an edge-tts Neural voice from an explicit name or text language hints."""
    if voice and 'Neural' in voice:
        return voice

    sample = (text or '')[:2000]
    if any(ch in sample for ch in 'åäöÅÄÖ') or len(_SV_HINT.findall(sample)) >= 2:
        return _EDGE_DEFAULT_VOICES['sv']

    if voice:
        base = voice.split('-')[0].lower() if '-' in voice else voice[:2].lower()
        if base in _EDGE_DEFAULT_VOICES:
            return _EDGE_DEFAULT_VOICES[base]

    return _EDGE_DEFAULT_VOICES['en']


async def _tts_xtts_sidecar(
    text: str,
    sample_path: Path,
    *,
    language: str = 'en',
    ...
    """Best-effort Coqui XTTS via sidecar. Returns True if file_path written."""
    import os

    base = (os.environ.get('XTTS_URL') or '').strip().rstrip('/')
    ...
    try:
        ...
        form.add_field('language', language[:8] or 'en')
        form.add_field(
            'speaker_wav',
            sample_path.read_bytes(),
            filename=sample_path.name or 'speaker.wav',
            content_type='audio/wav',
        )
        timeout = aiohttp.ClientTimeout(total=120)
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.post(f'{base}/synthesize', data=form) as resp:
                if resp.status >= 400:
                    body = await resp.text()
                    log.warning('XTTS sidecar HTTP %s: %s', resp.status, body[:200])
                    return False
                audio = await resp.read()
        if not audio or len(audio) < 100:
            return False
        # Sidecar returns wav; transcode to mp3 when possible for Call overlay.
        if not await asyncio.to_thread(transcode_audio_to_mp3, audio, 'audio/wav', file_path):
            file_path.write_bytes(audio)
        return file_path.is_file() and file_path.stat().st_size > 0
    except Exception as exc:  # noqa: BLE001
        log.warning('XTTS sidecar failed: %s', exc)
        return False


async def _tts_edge(request, payload, file_path, file_body_path, user):
    """Generate speech via Microsoft Edge neural TTS (edge-tts, no API key).

    Wave 9b: when voice-clone is enabled and XTTS_URL is up, try real clone first.
    """
    try:
        import edge_tts
        ...
        raise HTTPException(
            status_code=500,
            detail='edge-tts is not installed in this OpenWebUI image',
        ) from exc

    text = (payload.get('input') or '').strip()
    if not text:
        raise HTTPException(status_code=400, detail='Missing TTS input text')

    # Wave 6.3: apply private voice-clone profile when enabled for this user.
    clone_profile = None
    try:
        from open_webui.routers.spockify import _load_voice_clone_profile

        clone_profile = _load_voice_clone_profile(user.id) if user else None
    except Exception as exc:
        log.debug('voice clone profile load skipped: %s', exc)

    use_clone = bool(payload.get('use_clone')) or (
        clone_profile and clone_profile.get('enabled') and not payload.get('skip_clone')
    )

    # Wave 9.9 / 9b: real XTTS clone via sidecar when sample present.
    if use_clone and clone_profile and not payload.get('skip_xtts'):
        sample = Path(str(clone_profile.get('sample_path') or ''))
        lang_hint = 'sv' if any(ch in text for ch in 'åäöÅÄÖ') else 'en'
        if await _tts_xtts_sidecar(
            text, sample, language=lang_hint, file_path=file_path
        ):
            payload_meta = {
                **payload,
                'model': 'xtts',
                'voice': clone_profile.get('label') or 'xtts-clone',
            }
            async with aiofiles.open(file_body_path, 'w') as f:
                await f.write(json.dumps(payload_meta))
            log.info(
                'XTTS clone synthesized label=%s bytes=%s',
                clone_profile.get('label'),
            ...
            return FileResponse(
                ...
                headers={
                    'X-Spockify-Voice-Clone': '1',
                    'X-Spockify-TTS-Engine': 'xtts',
                },
            )

    if use_clone and clone_profile:
        if clone_profile.get('edge_voice'):
            payload = {**payload, 'voice': clone_profile['edge_voice']}
        if clone_profile.get('rate'):
            payload = {**payload, 'rate': clone_profile['rate']}
        if clone_profile.get('pitch'):
            payload = {**payload, 'pitch': clone_profile['pitch']}
        log.info(
            'edge-tts using voice-clone profile label=%s voice=%s',
            clone_profile.get('label'),
            clone_profile.get('edge_voice'),
        )

    configured = request.app.state.config.TTS_VOICE or ''
    voice = _resolve_edge_voice(payload.get('voice') or configured, text)
    rate = str(payload.get('rate') or _EDGE_DEFAULT_RATE)
    pitch = str(payload.get('pitch') or _EDGE_DEFAULT_PITCH)
    volume = str(payload.get('volume') or _EDGE_DEFAULT_VOLUME)
    want_stream = bool(payload.get('stream')) or str(
    ...
    try:
            ...
            async def audio_gen():
                ...
                async for chunk in communicate.stream():
                    if chunk.get('type') == 'audio' and chunk.get('data'):
                        ...
                        chunks.append(data)
                        yield data
                # Persist cache for subsequent non-stream hits
                try:
                    ...
                    log.warning('edge-tts stream cache write failed: %s', exc)

            log.info(
                f'edge-tts streaming voice={voice} rate={rate} pitch={pitch}'
            )
            return StreamingResponse(
                audio_gen(),
                media_type='audio/mpeg',
                headers={
                    'X-Spockify-TTS-Stream': '1',
                    'X-Spockify-Voice-Clone': '1' if use_clone and clone_profile else '0',
                },
            )
        ...
        async with aiofiles.open(file_body_path, 'w') as f:
            await f.write(json.dumps(payload))
        log.info(
            f'edge-tts synthesized voice={voice} rate={rate} pitch={pitch} '
...
async def speech(request: Request, user=Depends(get_verified_user)):
    # Empty engine = browser TTS preferred in the UI; /speech still serves
    # edge-tts so the frontend can fall back when no matching browser voice exists.
    engine = request.app.state.config.TTS_ENGINE or 'edge'
    ...
    # Strip markdown before cache key + synthesis so voices never say
    # "asterisk" / "hashtag" / etc. (Call + read-aloud share this path.)
    if isinstance(payload.get('input'), str):
        payload['input'] = sanitize_text_for_tts(payload['input'])
    ...
    try:
        ...
        if profile and profile.get('enabled'):
            clone_salt = (
                f"{profile.get('edge_voice')}|{profile.get('rate')}|{profile.get('pitch')}"
            ).encode('utf-8')
    except Exception:
        pass
    # Hash sanitized payload so markdown→speech changes bust stale caches.
    cache_body = json.dumps(payload, ensure_ascii=False, sort_keys=True).encode('utf-8')
    name = hashlib.sha256(
        cache_body
        + b'tts-md-sanitize-v1'
        + str(engine).encode('utf-8')
        + str(request.app.state.config.TTS_MODEL).encode('utf-8')
        + (
            f'{_EDGE_DEFAULT_RATE}|{_EDGE_DEFAULT_PITCH}|{_EDGE_DEFAULT_VOLUME}'.encode('utf-8')
            if engine in ('edge', '')
            else b''
        )
        + clone_salt
...
async def _transcribe_whisper(request, file_path, languages, file_dir, id):
    ...
    def _run():
        ...
        log.info(
            "Detected language '%s' with probability %f (beam_size=%s)"
            % (info.language, info.language_probability, WHISPER_BEAM_SIZE)
        )
...
async def get_available_voices(request) -> dict:
    ...
    if engine in ('edge', ''):
        return dict(_EDGE_CURATED_VOICES)
