...
async def spockify_status(request: Request, user=Depends(get_admin_user)):
    """Admin-only Spark status: Ollama models, ComfyUI, free RAM/GPU."""
    import os
...
async def unload_ollama_for_gpu(user=Depends(get_admin_user)):
    ...
    return {
        ...
        'note': (
            ...
            'run make free-gpu-for-training on the Spark host.'
        ),
    }
...
async def xtts_check(user=Depends(get_verified_user)):
    ...
    if enabled and xtts_url:
        ...
        try:
            ...
            engine = 'xtts'
            note = (
                'Coqui TTS importable in-process — prefer XTTS_URL sidecar on Spark.'
            )
...
#
_IDE_APPIMAGE_SPOCKIFY_VERSION = '0.9.17'
...
_IDE_APPIMAGE_LATEST_SHA256_X86_64 = (
    'a1e95ee5f66736d620ba11240ab9ecfd8dc61d26df0c7ab500caff7e67b6fd91'
)
# Debian package (amd64) — same product tree as the x86_64 AppImage.
_IDE_DEB_LATEST_SHA256_AMD64 = (
    'cbce9ed0c0bd708babf709fb9feda2f7be95f84372d5c8aab465c40cb283fdb9'
)
...
def _ide_appimage_download_url(spockify_version: str, arch: str) -> str:
    ...
    # Served from Spark nginx static at /downloads/ (also aliased at site root).
