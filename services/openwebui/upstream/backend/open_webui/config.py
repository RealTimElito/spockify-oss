...
# Duplex/Call STT latency: beam_size=1 is ~2–4× faster than default 5 with small quality cost.
try:
    WHISPER_BEAM_SIZE = max(1, int(os.getenv('WHISPER_BEAM_SIZE', '1')))
except ValueError:
    WHISPER_BEAM_SIZE = 1

# Keep faster-whisper resident after pod start (avoids multi-second cold STT on first Call).
WHISPER_WARM_ON_STARTUP = os.getenv('WHISPER_WARM_ON_STARTUP', 'true').lower() in (
    '1',
    'true',
    'yes',
    'on',
...
USER_PERMISSIONS_FEATURES_AUTOMATIONS = os.getenv('USER_PERMISSIONS_FEATURES_AUTOMATIONS', 'True').lower() == 'true'
