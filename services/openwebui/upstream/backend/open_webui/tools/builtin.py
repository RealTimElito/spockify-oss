...
async def generate_image(
    ...
    try:
        ...
        async def _emit_progress(description: str, done: bool = False):
        ...
        if __event_emitter__:
            await _emit_progress('Preparing GPU…', done=False)

        images = await image_generations(
            request=__request__,
            form_data=CreateImageForm(prompt=prompt),
            user=user,
            on_progress=_emit_progress if __event_emitter__ else None,
        )
