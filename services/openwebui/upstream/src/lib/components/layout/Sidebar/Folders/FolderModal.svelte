<script lang="ts">
	...
	const init = async () => {
		if (folderId) {
			...
			if (data.project_summary == null) data.project_summary = '';
		}
				...
				>
					...
					<div class="flex flex-col w-full mt-3">
						<div class=" mb-1 text-xs text-gray-500">
							{$i18n.t('Project summary')}
							<span class="text-gray-400">
								— {$i18n.t('remembered across chats in this project')}</span
							>
						</div>
						<div>
							<Textarea
								...
								placeholder={$i18n.t(
									'Short context for this project (goals, stack, constraints). Chats in this project see this summary.'
								)}
								maxSize={120}
								bind:value={data.project_summary}
							/>
						</div>
						<p class="mt-1.5 text-[11px] leading-snug text-gray-400 dark:text-gray-500">
							{$i18n.t(
