<script lang="ts">
	...
	import SpockifyNavIcon from '$lib/components/icons/SpockifyNavIcon.svelte';
	...
	const SPOCKIFY_MENU = [
		{ id: 'spockify', href: '/spockify', label: 'Spockify home' },
		{ id: 'spockify_agents', href: '/spockify/agents', label: 'Agents' },
		{ id: 'spockify_calendar', href: '/spockify/calendar', label: 'Calendar' },
		{ id: 'spockify_connectors', href: '/spockify/connectors', label: 'Connectors' },
		{ id: 'spockify_memory', href: '/spockify/memory', label: 'Memory' },
		{ id: 'spockify_lab', href: '/spockify/lab', label: 'Lab' },
		{ id: 'spockify_ghost', href: '/spockify/ghost', label: 'Ghost · AI IDE' }
	];
...
<Dropdown bind:show onOpenChange={handleDropdownChange} {align}>
	...
	<div slot="content">
		...
		>
			...
			{#if isSpockifyVerified()}
				{#each SPOCKIFY_MENU as item (item.id)}
					<div class="flex items-center w-full">
						<a
							href={item.href}
							draggable="false"
						...
						>
							<div class="self-center mr-3">
								<SpockifyNavIcon itemId={item.id} className="size-5" strokeWidth="1.5" />
							</div>
							<div class="self-center truncate">{$i18n.t(item.label)}</div>
						</a>
						{#if shiftKey}
							<Tooltip
								content={isPinned(item.id)
									? $i18n.t('Unpin from Sidebar')
									: $i18n.t('Pin to Sidebar')}
							>
								<button
									type="button"
									class="p-1 mr-1 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition"
									on:click|preventDefault|stopPropagation={() => togglePin(item.id)}
								>
									{#if isPinned(item.id)}
										<PinSlash className="size-3.5" strokeWidth="1.5" />
									{:else}
