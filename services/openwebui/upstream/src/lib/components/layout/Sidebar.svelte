<script lang="ts">
	...
	import SpockifyNavIcon from '../icons/SpockifyNavIcon.svelte';
	import { slide } from 'svelte/transition';
	import HotkeyHint from '../common/HotkeyHint.svelte';

	const BREAKPOINT = 768;
	/** Legacy multi-pin IDs (still valid if user re-pins from menu). */
	const SPOCKIFY_LEGACY_PINS = [
		'spockify_agents',
		'spockify_calendar',
		'spockify_connectors',
		'spockify_memory',
	...
	/** Prefer one hub pin + Calendar (less clutter). */
	const SPOCKIFY_PINNED_ITEMS = ['spockify', 'spockify_calendar'];
	const DEFAULT_PINNED_ITEMS = ['notes', 'workspace', ...SPOCKIFY_PINNED_ITEMS];
	...
	let showProjectsTip =
		typeof localStorage !== 'undefined' && !localStorage.getItem('spockifyProjectsTipSeen');
	...
	const isSpockifyVerified = () =>
		['admin', 'user', 'family', 'guest'].includes($user?.role ?? '');

	$: pinnedItems = $settings?.pinnedMenuItems ?? DEFAULT_PINNED_ITEMS;

	const migrateSpockifyPins = async () => {
		try {
			const saved = $settings?.pinnedMenuItems;
			if (!saved) return;
			// V3: collapse competing Spockify pins → hub + calendar.
			if (!$settings?.spockifySidebarPinsV3) {
				const withoutLegacy = saved.filter(
					(id) => !SPOCKIFY_LEGACY_PINS.includes(String(id)) && id !== 'spockify'
				);
				const next = [...withoutLegacy, 'spockify', 'spockify_calendar'];
				await settings.set({
					...$settings,
					pinnedMenuItems: next,
					spockifySidebarPinsV1: true,
					spockifySidebarPinsV2: true,
					spockifySidebarPinsV3: true
				});
			...
			if ($settings?.spockifySidebarPinsV2) return;
			let next = saved.some((id) => String(id).startsWith('spockify'))
				? [...saved]
				: [...saved, ...SPOCKIFY_PINNED_ITEMS];
			if (!next.includes('spockify_calendar') && !next.includes('spockify')) {
				next = [...next, ...SPOCKIFY_PINNED_ITEMS];
			}
			await settings.set({
				...$settings,
				pinnedMenuItems: next,
			...
			console.warn('spockify sidebar pin migrate', e);
		}
	};
	...
	const isMenuItemVisible = (id) => {
		switch (id) {
			...
			case 'spockify':
			case 'spockify_agents':
			case 'spockify_calendar':
			case 'spockify_connectors':
			case 'spockify_memory':
			case 'spockify_lab':
			case 'spockify_ghost':
				return isSpockifyVerified();
			default:
				return false;
		}
	};
	...
	const getMenuItemMeta = (id) => {
		const items = {
			...
			spockify: { label: 'Spockify', href: '/spockify', iconType: 'spockify' },
			spockify_agents: { label: 'Agents', href: '/spockify/agents', iconType: 'spockify' },
			spockify_calendar: {
				label: 'Calendar',
			...
			spockify_memory: { label: 'Memory', href: '/spockify/memory', iconType: 'spockify' },
			spockify_lab: { label: 'Lab', href: '/spockify/lab', iconType: 'spockify' },
			spockify_ghost: { label: 'Ghost · AI IDE', href: '/spockify/ghost', iconType: 'spockify' }
		};
		return items[id];
	};
							...
							<Tooltip content={$i18n.t(meta.label)} placement="right">
								...
								>
									<div class=" self-center flex items-center justify-center size-9">
										...
										{:else if itemId === 'spockify' || String(itemId).startsWith('spockify_')}
											<SpockifyNavIcon {itemId} className="size-4.5" strokeWidth="1.5" />
										{/if}
									</div>
								</a>
							</Tooltip>
								...
								>
									...
									>
										<div class="self-center">
											...
											{:else if itemId === 'spockify' || String(itemId).startsWith('spockify_')}
												<SpockifyNavIcon {itemId} className="size-4.5" strokeWidth="2" />
											{/if}
										</div>
				...
				{#if $config?.features?.enable_folders && ($user?.role === 'admin' || ($user?.permissions?.features?.folders ?? true))}
					...
					>
						{#if Object.keys(folders).length === 0}
							<div class="px-2.5 py-2 text-[11px] leading-snug text-gray-500 dark:text-gray-400">
								{$i18n.t(
									'Projects remember context across chats — add a summary when creating one'
								)}
								<button
									type="button"
									class="mt-1.5 block text-[11px] font-medium text-gray-700 dark:text-gray-200 hover:underline"
									on:click={() => {
										showCreateFolderModal = true;
									}}
								>
					...
					{#if showProjectsTip && Object.keys(folders).length === 0}
						<div
							class="mx-2 mt-1 mb-1 flex items-start gap-2 rounded-lg border border-sky-200/60 bg-sky-50/80 px-2.5 py-2 text-[11px] leading-snug text-sky-900 dark:border-sky-500/25 dark:bg-sky-400/10 dark:text-sky-100"
						>
							<span class="min-w-0 flex-1">
								{$i18n.t('Tip')}: {$i18n.t(
									'Use Projects (sidebar) for long-lived context — not just chat folders.'
								)}
							</span>
							<button
								type="button"
								class="shrink-0 text-sky-700/80 hover:text-sky-900 dark:text-sky-200/80 dark:hover:text-sky-50"
								aria-label={$i18n.t('Dismiss')}
								on:click={() => {
									try {
										localStorage.setItem('spockifyProjectsTipSeen', '1');
									} catch {
										/* ignore */
									}
									showProjectsTip = false;
								}}
							>
								×
							</button>
