...
<div {id} class={className}>
	...
	{:else}
		<!-- Tool call display — Cursor-like card chrome -->
		<div
			class="spockify-tool-call-card rounded-xl border border-gray-100/90 dark:border-gray-800/80 bg-gray-50/50 dark:bg-gray-900/35 {isExecuting
				? 'border-sky-200/50 dark:border-sky-500/25'
				: ''}"
		>
			<!-- svelte-ignore a11y-no-static-element-interactions -->
			<div
				class="{buttonClassName} cursor-pointer w-full max-w-full font-medium flex items-center gap-1.5 px-2.5 py-1.5 {isExecuting
					? 'shimmer'
					: ''}"
				on:pointerup={() => {
					open = !open;
			...
			{#if open}
				<div transition:slide={{ duration: 300, easing: quintOut, axis: 'y' }}>
					<div class="border-t border-gray-100/80 dark:border-gray-800/60 mx-0 my-0 p-3 space-y-3">
						<!-- Input -->
						{#if args}
							<div>
								<div
