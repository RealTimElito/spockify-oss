<script>
	...
	import SpockifyStatus from './Settings/SpockifyStatus.svelte';
	import SpockifyUsage from './Settings/SpockifyUsage.svelte';
	import SpockifyTools from './Settings/SpockifyTools.svelte';
	import SpockifyConnectors from './Settings/SpockifyConnectors.svelte';
	import SpockifyEval from './Settings/SpockifyEval.svelte';
	import SpockifyFamily from './Settings/SpockifyFamily.svelte';
	...
	const allSettings = [
		...
		{
			id: 'spockify',
			title: 'Spockify Status',
			route: '/admin/settings/spockify',
			keywords: [
				'spockify',
				'status',
				'ollama',
		...
		{
			id: 'spockify-usage',
			title: 'Spockify Usage',
			route: '/admin/settings/spockify-usage',
			keywords: [
				'spockify',
				'usage',
				'spend',
		...
		{
			id: 'spockify-tools',
			title: 'Spockify Tools',
			route: '/admin/settings/spockify-tools',
			keywords: [
				'spockify',
				'tools',
				'marketplace',
		...
		{
			id: 'spockify-connectors',
			title: 'Connectors (legacy migrate)',
			route: '/admin/settings/spockify-connectors',
			keywords: [
				'spockify',
				'connectors',
				'calendar',
		...
		},
		{
			id: 'spockify-eval',
			title: 'Eval board',
			route: '/admin/settings/spockify-eval',
			keywords: ['spockify', 'eval', 'arena', 'benchmark', 'scores']
		},
		{
			id: 'spockify-family',
			title: 'Family / guest',
			route: '/admin/settings/spockify-family',
			keywords: ['spockify', 'family', 'guest', 'caps', 'roles']
		}
	];
	...
	const setFilteredSettings = () => {
		...
		try {
			privacyOn =
				localStorage.getItem('spockifyPrivacyMode') === '1' ||
				localStorage.getItem('spockifyPrivacyMode') === 'true';
		} catch {
			/* ignore */
		}
		filteredSettings = allSettings.filter((tab) => {
			if (privacyOn && tab.id === 'spockify-usage') return false;
...
<div class="flex flex-col lg:flex-row w-full h-full pb-2 lg:space-x-4">
		...
		{#each filteredSettings as tab (tab.id)}
			...
			>
				<div class=" self-center mr-2">
					...
					{:else if tab.id === 'spockify' || tab.id === 'spockify-usage' || tab.id === 'spockify-tools' || tab.id === 'spockify-connectors' || tab.id === 'spockify-eval' || tab.id === 'spockify-family'}
						<svg
							xmlns="http://www.w3.org/2000/svg"
							viewBox="0 0 16 16"
							fill="currentColor"
						...
						>
							<path
								fill-rule="evenodd"
								d="M8 1.75a.75.75 0 0 1 .692.462l1.41 3.393 3.664.293a.75.75 0 0 1 .428 1.317l-2.78 2.365.83 3.566a.75.75 0 0 1-1.12.814L8 11.847l-3.124 1.823a.75.75 0 0 1-1.12-.814l.83-3.566-2.78-2.365a.75.75 0 0 1 .427-1.317l3.665-.293 1.41-3.393A.75.75 0 0 1 8 1.75Z"
								clip-rule="evenodd"
							/>
						</svg>
					{/if}
	...
	>
		...
		{:else if selectedTab === 'pipelines'}
			...
			/>
		{:else if selectedTab === 'spockify'}
			<SpockifyStatus />
		{:else if selectedTab === 'spockify-usage'}
			{#if typeof localStorage !== 'undefined' && (localStorage.getItem('spockifyPrivacyMode') === '1' || localStorage.getItem('spockifyPrivacyMode') === 'true')}
				<div class="text-sm text-gray-500 dark:text-gray-400 p-4">
					{$i18n.t('Local-only mode is on — spend / usage display is hidden.')}
				</div>
			{:else}
				<SpockifyUsage />
			{/if}
		{:else if selectedTab === 'spockify-tools'}
			<SpockifyTools />
		{:else if selectedTab === 'spockify-connectors'}
			<SpockifyConnectors />
		{:else if selectedTab === 'spockify-eval'}
			<SpockifyEval />
		{:else if selectedTab === 'spockify-family'}
			<SpockifyFamily />
		{/if}
	</div>
</div>
