<script lang="ts">
	...
	const DEFAULT_ACTIONS = [
		...
		{
			id: 'ghost',
			label: $i18n.t('Open in Ghost'),
			icon: CommandLine,
			openGhost: true
		}
	];

	const openInGhost = (code: string) => {
		try {
			sessionStorage.setItem('spockifyGhostSeed', code);
		} catch {
			/* ignore */
		}
		goto('/spockify/ghost');
	...
	const actionHandler = (actionId) => {
		...
		if (selectedAction.openGhost) {
			openInGhost(selectedText);
			return;
		}
