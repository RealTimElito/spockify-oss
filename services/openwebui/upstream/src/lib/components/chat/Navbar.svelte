	...
	<div class="flex items-center w-full pl-1.5 pr-1">
		...
		<div class=" flex max-w-full w-full mx-auto px-1.5 md:px-2 pt-0.5 bg-transparent">
			<div class="flex items-center w-full max-w-full">
				...
				<div
					class="flex-1 overflow-hidden max-w-full mt-0.5 py-0.5 min-w-0
			{$showSidebar ? 'ml-1' : ''}
			"
				>
				...
				<div class="self-start flex flex-none items-center text-gray-600 dark:text-gray-400">
					...
					{#if shareEnabled && chat && (chat.id || $temporaryChatEnabled)}
						<div class="flex flex-row items-center shrink-0">
							<Tooltip
								content={$i18n.t(
									'Vault: Fernet at rest — not E2E against a hostile server'
								)}
							>
								<button
									type="button"
									class="flex flex-row items-center gap-1.5 shrink-0 whitespace-nowrap cursor-pointer px-2 py-2 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-850 transition text-xs text-gray-600 dark:text-gray-300"
									aria-label={$i18n.t('Vault')}
									on:click={async () => {
										if (!chat?.id) return;
										const passphrase = prompt(
											$i18n.t(
												'Vault passphrase — NOT end-to-end: server sees plaintext while unlocked; protects idle DB dumps only'
											)
										);
										if (!passphrase) return;
										try {
											const { vaultLockChat, getVaultStatus } = await import(
												'$lib/apis/spockify'
											);
											const st = await getVaultStatus(localStorage.token, chat.id);
											const lock = !st?.locked;
											const res = await vaultLockChat(localStorage.token, {
												chat_id: chat.id,
												passphrase,
												lock
											});
											toast.success(
												lock
													? $i18n.t('Chat locked in vault')
													: $i18n.t('Chat unlocked')
											);
											if (res?.threat_model) console.info('[vault]', res.threat_model);
											location.reload();
										} catch (e) {
											toast.error(`${e}`);
										}
								...
								>
									...
									<span class="hidden sm:inline">{$i18n.t('Vault')}</span>
								</button>
							</Tooltip>
							<Menu
								{chat}
