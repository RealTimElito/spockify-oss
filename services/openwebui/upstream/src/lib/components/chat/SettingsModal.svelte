<script lang="ts">
	...
	import SpockifyConnectors from './Settings/SpockifyConnectors.svelte';
	import SpockifyScheduledAgents from '../admin/Settings/SpockifyScheduledAgents.svelte';
	import SpockifyMemory from '../admin/Settings/SpockifyMemory.svelte';
	import SpockifySkills from '../admin/Settings/SpockifySkills.svelte';
	import SpockifyAgentsReplay from '../admin/Settings/SpockifyAgentsReplay.svelte';
	import SpockifyWave10 from '../admin/Settings/SpockifyWave10.svelte';
	import SpockifyGhostWriter from './SpockifyGhostWriter.svelte';
	...
	const allSettings: SettingsTab[] = [
		...
		},
		{
			id: 'spockify_memory',
			title: 'Memory',
			keywords: ['memory', 'project', 'summary', 'session', 'digest', 'spockify']
		},
		{
			id: 'spockify_skills',
			title: 'Skills packs',
			keywords: ['skills', 'prompt packs', 'SKILL.md', 'spockify']
		},
		{
			id: 'spockify_agents_replay',
			title: 'Agent run replay',
			keywords: [
				'replay',
				'timeline',
				'parallel',
		...
		{
			id: 'spockify_ghost',
			title: 'Ghost writer IDE',
			keywords: ['ghost', 'monaco', 'ide', 'code', 'spockify']
		},
		{
			id: 'audio',
			title: 'Audio',
		...
		<div class="flex flex-col md:flex-row w-full pt-1 pb-4">
			...
			>
				...
				{#if filteredSettings.length > 0}
					{#each filteredSettings as tabId (tabId)}
						...
						{:else if tabId === 'spockify_connectors'}
							<button
								role="tab"
								aria-controls="tab-spockify-connectors"
								aria-selected={selectedTab === 'spockify_connectors'}
								class={`px-0.5 md:px-2.5 py-1 min-w-fit rounded-xl flex-1 md:flex-none flex text-left transition
								${
									selectedTab === 'spockify_connectors'
										? ($settings?.highContrastMode ?? false)
											? 'dark:bg-gray-800 bg-gray-200'
											: ''
										: ($settings?.highContrastMode ?? false)
							...
							>
								...
								<div class=" self-center">{$i18n.t('Your connectors')}</div>
							</button>
						{:else if tabId === 'spockify_agents'}
							<button
								role="tab"
								aria-controls="tab-spockify-agents"
								aria-selected={selectedTab === 'spockify_agents'}
								class={`px-0.5 md:px-2.5 py-1 min-w-fit rounded-xl flex-1 md:flex-none flex text-left transition
								${
									selectedTab === 'spockify_agents'
										? ($settings?.highContrastMode ?? false)
											? 'dark:bg-gray-800 bg-gray-200'
											: ''
										: ($settings?.highContrastMode ?? false)
							...
							>
								...
								<div class=" self-center">{$i18n.t('Scheduled agents')}</div>
							</button>
						{:else if tabId === 'spockify_memory'}
							<button
								role="tab"
								aria-controls="tab-spockify-memory"
								aria-selected={selectedTab === 'spockify_memory'}
								class={`px-0.5 md:px-2.5 py-1 min-w-fit rounded-xl flex-1 md:flex-none flex text-left transition
								${
									selectedTab === 'spockify_memory'
										? ($settings?.highContrastMode ?? false)
											? 'dark:bg-gray-800 bg-gray-200'
											: ''
										: ($settings?.highContrastMode ?? false)
						...
						{:else if tabId === 'spockify_skills'}
							<button
								role="tab"
								aria-controls="tab-spockify-skills"
								aria-selected={selectedTab === 'spockify_skills'}
								class={`px-0.5 md:px-2.5 py-1 min-w-fit rounded-xl flex-1 md:flex-none flex text-left transition
								${
									selectedTab === 'spockify_skills'
										? ($settings?.highContrastMode ?? false)
											? 'dark:bg-gray-800 bg-gray-200'
											: ''
										: ($settings?.highContrastMode ?? false)
							...
							>
								...
								<div class=" self-center">{$i18n.t('Skills packs')}</div>
							</button>
						{:else if tabId === 'spockify_agents_replay'}
							<button
								role="tab"
								aria-controls="tab-spockify-agents-replay"
								aria-selected={selectedTab === 'spockify_agents_replay'}
								class={`px-0.5 md:px-2.5 py-1 min-w-fit rounded-xl flex-1 md:flex-none flex text-left transition
								${
									selectedTab === 'spockify_agents_replay'
										? ($settings?.highContrastMode ?? false)
											? 'dark:bg-gray-800 bg-gray-200'
											: ''
										: ($settings?.highContrastMode ?? false)
							...
							>
								...
								<div class=" self-center">{$i18n.t('Agent run replay')}</div>
							</button>
						{:else if tabId === 'spockify_lab'}
							<button
								role="tab"
								aria-controls="tab-spockify-lab"
								aria-selected={selectedTab === 'spockify_lab'}
								class={`px-0.5 md:px-2.5 py-1 min-w-fit rounded-xl flex-1 md:flex-none flex text-left transition
								${
									selectedTab === 'spockify_lab'
										? ($settings?.highContrastMode ?? false)
											? 'dark:bg-gray-800 bg-gray-200'
											: ''
										: ($settings?.highContrastMode ?? false)
							...
							>
								...
								<div class=" self-center">{$i18n.t('Spockify lab')}</div>
							</button>
						{:else if tabId === 'spockify_ghost'}
							<button
								role="tab"
								aria-controls="tab-spockify-ghost"
								aria-selected={selectedTab === 'spockify_ghost'}
								class={`px-0.5 md:px-2.5 py-1 min-w-fit rounded-xl flex-1 md:flex-none flex text-left transition
								${
									selectedTab === 'spockify_ghost'
										? ($settings?.highContrastMode ?? false)
											? 'dark:bg-gray-800 bg-gray-200'
											: ''
										: ($settings?.highContrastMode ?? false)
							...
							>
								...
								<div class=" self-center">{$i18n.t('Ghost writer IDE')}</div>
							</button>
						{:else if tabId === 'audio'}
							<button
								role="tab"
			...
			>
				...
				{:else if selectedTab === 'personalization'}
					...
					/>
				{:else if selectedTab === 'spockify_connectors'}
					<SpockifyConnectors />
				{:else if selectedTab === 'spockify_agents'}
					<SpockifyScheduledAgents />
				{:else if selectedTab === 'spockify_memory'}
					<SpockifyMemory />
				{:else if selectedTab === 'spockify_skills'}
					<SpockifySkills />
				{:else if selectedTab === 'spockify_agents_replay'}
					<SpockifyAgentsReplay />
				{:else if selectedTab === 'spockify_lab'}
					<SpockifyWave10 />
				{:else if selectedTab === 'spockify_ghost'}
					<div class="flex flex-col gap-2">
						<div class="text-xs text-gray-500">
							{$i18n.t('Monaco embed — Ghost writer IDE for your coding sessions.')}
						</div>
						<SpockifyGhostWriter open={true} />
					</div>
