<script lang="ts">
	...
	import {
		...
		IMAGE_STYLE_OPTIONS,
		promptSuggestsImageGeneration,
		type SpockifyImageAspect,
		type SpockifyImageStyle
	} from '$lib/utils/imageOptions';
	import {
		VIDEO_DURATION_OPTIONS,
		promptSuggestsVideoGeneration,
		type SpockifyVideoDuration
	} from '$lib/utils/videoOptions';
	...
	export let canvasEnabled = false;
	...
	/** Derived: Multitask → parallel-agents fan-out. */
	export let spockifyParallelAgents = false;
	export let onSpockifyParallelAgentsChange: (on: boolean) => void = () => {};
	/** Active parallel-agents run (e.g. natural-language /agents) for the bar. */
	export let activeAgentsRun: any = null;
	export let onCancelAgentsRun: (() => void) | null = null;
	export let agentsCancelBusy = false;
	/** W4.10 privacy / local-only mode. */
	export let spockifyPrivacyMode = false;
	export let onSpockifyPrivacyModeChange: (on: boolean) => void = () => {};
	/** Q4 Flux aspect / style (session). */
	export let spockifyImageAspect: SpockifyImageAspect = 'square';
	export let onSpockifyImageAspectChange: (aspect: SpockifyImageAspect) => void = () => {};
	export let spockifyImageStyle: SpockifyImageStyle = '';
	export let onSpockifyImageStyleChange: (style: SpockifyImageStyle) => void = () => {};
	/** LTX video length (session). */
	export let spockifyVideoDuration: SpockifyVideoDuration = 'default';
	export let onSpockifyVideoDurationChange: (duration: SpockifyVideoDuration) => void = () => {};
	...
	export const setText = async (text?: string, cb?: (text: string) => void) => {
		...
		if (chatInput) {
			...
			// Skip focus during Call — focusing chat-input opens the mobile keyboard.
			if (!$showCallOverlay) {
				chatInputElement?.focus();
			} else {
	...
	const cycleSpockifySearchMode = () => {
		...
		const next =
			spockifySearchMode === 'auto' ? 'on' : spockifySearchMode === 'on' ? 'off' : 'auto';
		spockifySearchMode = next;
	...
	let showImageOptionChips = false;
	$: showImageOptionChips =
		showImageGenerationButton &&
		(imageGenerationEnabled || promptSuggestsImageGeneration(prompt));

	// Same pattern as image: Video toggle (Integrations) or prompt intent.
	let showVideoGenerationButton = false;
	$: showVideoGenerationButton = !!$config?.features?.enable_video_generation;

	let showVideoOptionChips = false;
	$: showVideoOptionChips =
		showVideoGenerationButton &&
		(videoGenerationEnabled || promptSuggestsVideoGeneration(prompt));

	const chipClass = (active: boolean) =>
		active
			? 'px-2 py-[3px] text-[11px] rounded-full transition-colors text-sky-600 dark:text-sky-300 bg-sky-50 hover:bg-sky-100 dark:bg-sky-400/10 border border-sky-200/40 dark:border-sky-500/20'
			: 'px-2 py-[3px] text-[11px] rounded-full transition-colors text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 border border-transparent';
				...
				<div class="">
					...
					>
						...
						<div
							...
							class="spockify-composer-box flex-1 flex flex-col relative w-full shadow-lg rounded-3xl border {$temporaryChatEnabled
							...
							class:spockify-working={isActive}
						...
						>
							...
							{#if showImageOptionChips}
								<div
									class="px-3 pb-1 pt-0.5 flex flex-wrap items-center gap-1.5 text-xs"
									aria-label="Image generation options"
								>
									<span class="text-[10px] uppercase tracking-wide text-gray-400 dark:text-gray-500 mr-0.5"
										>Aspect</span
									>
									{#each IMAGE_ASPECT_OPTIONS as opt}
										<button
											type="button"
											class={chipClass(spockifyImageAspect === opt.id)}
											aria-pressed={spockifyImageAspect === opt.id}
											on:click|preventDefault={() => onSpockifyImageAspectChange(opt.id)}
										>
											{opt.label}
										</button>
									{/each}
									<span
										class="mx-1 w-px h-3 bg-gray-200 dark:bg-gray-700 self-center"
										aria-hidden="true"
									></span>
									<span class="text-[10px] uppercase tracking-wide text-gray-400 dark:text-gray-500 mr-0.5"
										>Style</span
									>
									{#each IMAGE_STYLE_OPTIONS as opt}
										<button
											type="button"
											class={chipClass(spockifyImageStyle === opt.id)}
											aria-pressed={spockifyImageStyle === opt.id}
											on:click|preventDefault={() => onSpockifyImageStyleChange(opt.id)}
										>
											{opt.label}
										</button>
									{/each}
							...
							{#if showVideoOptionChips}
								<div
									class="px-3 pb-1 pt-0.5 flex flex-wrap items-center gap-1.5 text-xs"
									aria-label="Video generation options"
								>
									<span class="text-[10px] uppercase tracking-wide text-gray-400 dark:text-gray-500 mr-0.5"
										>Length</span
									>
									{#each VIDEO_DURATION_OPTIONS as opt}
										<button
											type="button"
											title={opt.id === 'short'
												? '~1s (25 frames)'
												: opt.id === 'long'
													? '~10s (241 frames)'
													: '~2.7s (65 frames)'}
											class={chipClass(spockifyVideoDuration === opt.id)}
											aria-pressed={spockifyVideoDuration === opt.id}
											on:click|preventDefault={() => onSpockifyVideoDurationChange(opt.id)}
										>
											{opt.label}
										</button>
									{/each}
								...
								>
									...
									>
										<button
											...
											class="bg-transparent hover:bg-gray-100 text-gray-700 dark:text-white dark:hover:bg-gray-800 rounded-full size-10 sm:size-8 flex justify-center items-center outline-hidden focus:outline-hidden shrink-0"
										...
										<div class="hidden sm:flex items-center gap-0.5 shrink-0">
											<Tooltip
												content={spockifyPrivacyMode
													...
													: spockifySearchMode === 'auto'
														? 'Web search: automatic (heuristics)'
														: spockifySearchMode === 'on'
															? 'Web search: forced on'
															: 'Answer without web (no SearXNG)'}
												placement="top"
											>
												<button
													type="button"
													aria-label={spockifySearchLabel(spockifySearchMode)}
													on:click|preventDefault={cycleSpockifySearchMode}
													disabled={spockifyPrivacyMode}
													class="px-2 py-[5px] flex gap-1 items-center text-xs rounded-full transition-colors duration-300 focus:outline-hidden {spockifySearchMode ===
													'on'
														? 'text-sky-600 dark:text-sky-300 bg-sky-50 hover:bg-sky-100 dark:bg-sky-400/10 border border-sky-200/40 dark:border-sky-500/20'
														: spockifySearchMode === 'off'
															? 'text-gray-500 dark:text-gray-400 bg-gray-100/80 dark:bg-gray-800/60 border border-gray-200/60 dark:border-gray-700/50 line-through decoration-gray-400/80'
												...
												>
													...
													<span class="whitespace-nowrap"
														>{spockifySearchLabel(spockifySearchMode)}</span
													>
												</button>
											</Tooltip>
										...
										<div class="flex sm:hidden items-center shrink-0">
											...
											>
												<button
													...
													on:click|preventDefault={cycleSpockifySearchMode}
													disabled={spockifyPrivacyMode}
													class="rounded-full size-10 flex justify-center items-center outline-hidden focus:outline-hidden disabled:opacity-50 {spockifySearchMode ===
													'on'
														? 'text-sky-600 dark:text-sky-300 bg-sky-50 dark:bg-sky-400/10'
														: spockifySearchMode === 'off'
															? 'text-gray-500 dark:text-gray-400 bg-gray-100/80 dark:bg-gray-800/60'
									...
									{#if selectedModelIds.length === 1 && $models.find((m) => m.id === selectedModelIds[0])?.has_user_valves}
										<div class="ml-1 hidden sm:flex gap-1.5 shrink-0">
									...
									<div class="ml-0.5 sm:ml-1 flex gap-1.5 shrink-0 overflow-x-auto scrollbar-none">
										...
										{#if videoGenerationEnabled}
											<Tooltip content="Video" placement="top">
												<button
													on:click|preventDefault={() =>
										...
										{#if canvasEnabled}
											<Tooltip content="Canvas" placement="top">
												<button
													on:click|preventDefault={() => (canvasEnabled = !canvasEnabled)}
													type="button"
													class="group p-[7px] flex gap-1.5 items-center text-sm rounded-full transition-colors duration-300 focus:outline-hidden max-w-full overflow-hidden {canvasEnabled
														? ' text-sky-500 dark:text-sky-300 bg-sky-50 hover:bg-sky-100 dark:bg-sky-400/10 dark:hover:bg-sky-700/10 border border-sky-200/40 dark:border-sky-500/20'
														: 'bg-transparent text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 '}"
												>
									...
									{#if isActive && prompt === '' && files.length === 0}
										<div class=" flex items-center">
											<Tooltip content={$i18n.t('Stop')}>
												<button
													class="spockify-send-round transition rounded-full p-2 sm:p-1.5"
											...
											{#if $_user?.role === 'admin' || ($_user?.permissions?.chat?.stt ?? true)}
												...
												<Tooltip content={$i18n.t('Dictate')}>
													<button
														...
														class=" text-gray-600 dark:text-gray-300 hover:text-gray-700 dark:hover:text-gray-200 transition rounded-full p-2 sm:p-1.5 self-center mr-0.5"
														type="button"
														on:click={async () => {
															try {
										...
										{#if showCallButton}
											<div class="flex items-center">
												...
												<Tooltip content={$i18n.t('Voice mode')}>
													<button
														class="spockify-send-round transition rounded-full p-2 sm:p-1.5 self-center"
														type="button"
														on:click={async () => {
															...
															try {
																...
																// Keep soft keyboard closed in Call / duplex voice mode.
...
<style>
	/*
	 * Neon sweeping working border — Spockify chrome (mirrors IDE chat.css /
	 * Cursor chat-input-container.working). Do NOT replace with Cursor’s plain border.
	 */
	@property --spk-input-anim-angle {
		syntax: '<angle>';
		inherits: false;
		initial-value: 135deg;
	}
	@keyframes spk-input-working-spin {
		from {
			--spk-input-anim-angle: 135deg;
		}
		to {
			--spk-input-anim-angle: 495deg;
		}
	}
	.spockify-composer-box {
		--spk-input-working: #3dfff0;
		isolation: isolate;
	}
	.spockify-composer-box.spockify-working {
		border-color: transparent !important;
	}
	.spockify-composer-box.spockify-working::before,
	.spockify-composer-box.spockify-working::after {
		content: '';
		position: absolute;
		inset: -1px;
		...
		pointer-events: none;
		z-index: 0;
		animation: spk-input-working-spin 4s linear infinite;
	}
	.spockify-composer-box.spockify-working::before {
		padding: 1px;
		background: conic-gradient(
			from var(--spk-input-anim-angle),
			transparent 0deg,
			color-mix(in srgb, var(--spk-input-working) 90%, transparent) 20deg,
			var(--spk-input-working) 30deg,
			color-mix(in srgb, var(--spk-input-working) 60%, transparent) 50deg,
			transparent 90deg,
			transparent 360deg
		);
		-webkit-mask:
	...
	.spockify-composer-box.spockify-working::after {
		...
		background: conic-gradient(
			from var(--spk-input-anim-angle),
			transparent 0deg,
			color-mix(in srgb, var(--spk-input-working) 55%, transparent) 25deg,
			color-mix(in srgb, var(--spk-input-working) 30%, transparent) 50deg,
			transparent 90deg,
			transparent 360deg
		);
		-webkit-mask:
	...
	/* Keep toolbar/input above the beam layers */
	.spockify-composer-box > :global(*) {
		position: relative;
		z-index: 1;
	}

	/* Neutral send (web chat — no IDE agent-mode tints) */
	.spockify-send-round {
		color: white;
		background: rgb(17 24 39);
	}
	:global(.dark) .spockify-send-round {
		color: rgb(17 24 39);
		background: white;
	}
	.spockify-send-round.spockify-send-disabled {
		opacity: 0.45;
		background: rgb(229 231 235) !important;
		color: white !important;
	}
	:global(.dark) .spockify-send-round.spockify-send-disabled {
