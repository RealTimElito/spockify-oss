		...
		if (transcribe) {
			if ($config.audio.stt.engine === 'web' || ($settings?.audio?.stt?.engine ?? '') === 'web') {
				if ('SpeechRecognition' in window || 'webkitSpeechRecognition' in window) {
					...
					// Prefer explicit STT language; else UI locale. Leave unset only if neither
					// is available (browser default). Web Speech has weak auto-detect.
					const sttLang = ($settings?.audio?.stt?.language || '').trim();
					const uiLang = ($i18n as { language?: string })?.language || '';
					const isoToBcp47: Record<string, string> = {
						sv: 'sv-SE',
						en: 'en-US',
						de: 'de-DE',
						fr: 'fr-FR',
					...
					if (sttLang) {
						speechRecognition.lang =
							sttLang.length === 2 ? isoToBcp47[sttLang] || sttLang : sttLang;
					} else if (uiLang) {
						speechRecognition.lang = uiLang;
					}
