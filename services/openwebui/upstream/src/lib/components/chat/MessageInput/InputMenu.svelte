<script lang="ts">
	...
	import { config, user, tools as _tools, knowledge } from '$lib/stores';
	...
	/** Regular / Plan / Multitask — shown when Spockify models are selected. */
	export let showComposerModes = false;
	...
	const selectComposerMode = (id: ComposerUiMode) => {
		composerMode = id;
		onComposerModeChange(id);
		onParallelAgentsChange(id === 'multitask');
		show = false;
	};
			...
			{#if tab === ''}
				<div in:fly={{ x: -20, duration: 150 }}>
					...
					{#if showComposerModes}
						...
						{#each COMPOSER_MODE_META as m (m.id)}
							<button
								type="button"
								...
								composerModeId
									...
									: ''}"
								aria-pressed={m.id === composerModeId}
								on:click={() => selectComposerMode(m.id)}
							>
								<span
									class="inline-flex size-4 shrink-0 items-center justify-center spockify-mode-{m.id}"
									aria-hidden="true"
								>
									{#if m.id === 'plan'}
									...
									{:else if m.id === 'multitask'}
										<svg width="14" height="14" viewBox="0 0 16 16" fill="none">
											<circle cx="5.5" cy="8" r="2.2" stroke="currentColor" stroke-width="1.2" />
											<circle cx="10.5" cy="8" r="2.2" stroke="currentColor" stroke-width="1.2" />
										</svg>
									{:else}
										<svg width="14" height="14" viewBox="0 0 16 16" fill="none">
											<path
