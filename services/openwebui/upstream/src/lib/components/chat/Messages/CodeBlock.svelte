		...
		{:else}
			...
			>
				...
				<div class="flex items-center gap-0.5 shrink-0">
					...
					{#if preview && (shouldAutoOpenCanvas(lang, code) || isCanvasDocLang(lang) || isSubstantialCodeCanvas(lang, code) || isLongPlainFence(lang, code))}
						<button
							class="flex gap-1 items-center run-code-button bg-none border-none transition rounded-md px-1.5 py-0.5 bg-white dark:bg-black"
						...
						>
							<div>
								{['html', 'svg'].includes(lang.toLowerCase())
									? $i18n.t('Preview')
									: 'Canvas'}
							</div>
						</button>
