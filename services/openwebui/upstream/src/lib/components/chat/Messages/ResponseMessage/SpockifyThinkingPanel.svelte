<script lang="ts">
	...
	$: phaseLabel = (() => {
		...
		// Mid-thought SPAWN (not Heavy ensemble): keep header wording clean.
		if (runActive && workers.length > 0 && thinking !== 'heavy') {
			if (agents?.status === 'synthesizing') return 'Merging workers…';
