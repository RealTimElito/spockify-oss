<script lang="ts">
	...
	import { cancelAgentRun, cancelAgentWorker } from '$lib/apis/spockify';
	...
	let model = null;
	$: displayModelId = message?.arena ? message.model : (message?.selectedModelId ?? message.model);
	$: model = $models.find((m) => m.id === displayModelId);
	...
	$: isWebPhotoMessage =
		...
		(statusEntries.some((s) => s?.action === 'web_image') ||
			/^Here'?s a photo of /i.test((message?.content ?? '').trim()));
	...
	$: canRequestAnotherWebPhoto =
		!readOnly && message?.done && messageImages.length > 0 && isWebPhotoMessage;

	let variationBusy = false;

	const stripVariationPrefix = (text: string) =>
		text
			.replace(
				/^(?:generate\s+(?:an?\s+)?(?:image\s+)?variation(?:\s+of)?[:\s]*|make\s+another\s+(?:one\s+)?(?:like\s+this|variation)(?:\s+of)?[:\s]*)/i,
				''
			)
			.trim();

	const requestImageVariation = async () => {
		if (variationBusy || !canRequestImageVariation) return;
		const parent = history?.messages?.[message?.parentId];
		let base = stripVariationPrefix((parent?.content || '').trim());
		if (!base) {
			base = 'the previous image';
		}
		const variationPrompt = `Generate an image variation: ${base}`;
		variationBusy = true;
		try {
			await submitMessage(message.id, variationPrompt);
		} finally {
			variationBusy = false;
		}
	};

	const requestAnotherWebPhoto = async () => {
		if (variationBusy || !canRequestAnotherWebPhoto) return;
		variationBusy = true;
		try {
			await submitMessage(message.id, 'Can you show me another one?');
		} finally {
			variationBusy = false;
		}
	};
	...
	const cancelParallelAgentsRun = async () => {
		const runId = message.spockifyAgents?.id;
		if (!runId || !$user?.token || agentsCancelBusy) return;
		agentsCancelBusy = true;
		try {
			const res = await cancelAgentRun($user.token, runId);
			if (res?.run) {
				message.spockifyAgents = res.run;
				history.messages[messageId] = message;
			}
			toast.success($i18n.t('Parallel agents cancelled'));
		} catch (e) {
			toast.error(`${e}`);
		} finally {
	...
	const cancelParallelAgentWorker = async (workerId: string) => {
		const runId = message.spockifyAgents?.id;
		if (!runId || !workerId || !$user?.token || agentsCancelBusy) return;
		agentsCancelBusy = true;
		try {
			const res = await cancelAgentWorker($user.token, runId, workerId);
			if (res?.run) {
				message.spockifyAgents = res.run;
				history.messages[messageId] = message;
			}
	...
	const speak = async () => {
		...
		const speakViaServer = async (voiceId: string) => {
		...
		// Spockify: always prefer server edge-tts (Multilingual Neural) with auto lang.
		if (isServerEdgeTtsEngine($config.audio.tts.engine)) {
			const speechLang = detectSpeechLanguage(content);
			const voiceId = getVoiceId();
			const edgeByLang = $settings?.audio?.tts?.edgeVoiceByLang;
			const edgeVoice = resolveEdgeVoice(speechLang, edgeByLang, voiceId);
			logSpeechVoiceChoice(speechLang, undefined, edgeVoice);
			await speakViaServer(edgeVoice);
		} else {
			await speakViaServer(getVoiceId());
		}
	};
		...
		<div class="flex-auto w-0 pl-1 relative">
			<Name>
				...
				{#if message.model === 'spockify-auto' && message.spockifyWorker}
					{@const whyBits = [
						message.spockifyRoutingPath
							...
							: '',
						message.spockifyReason || '',
						message.spockifyWebSearch ? 'web search' : ''
					].filter(Boolean)}
					{@const whyTooltip =
						whyBits.length > 0
							? whyBits.join('<br/>')
							: message.spockifyWebSearch
								? `${message.spockifyWorker} · web search`
								: message.spockifyWorker}
					<Tooltip content={whyTooltip} placement="top-start">
						<span
							class="text-xs font-normal text-gray-400 dark:text-gray-500 line-clamp-1 shrink-0 cursor-help"
							title={whyBits.join(' · ') || message.spockifyWorker}
						>
							{message.spockifyWorker}{#if message.spockifyWebSearch} · web{/if}
						</span>
					</Tooltip>
				{/if}
				...
				{#if message.model === 'spockify-agents' || message.spockifyAgents}
					...
					{@const agentsTok =
						(agentsHud?.total_tokens ?? 0) ||
						(agentsHud?.prompt_tokens || 0) + (agentsHud?.completion_tokens || 0)}
					<span
						class="text-xs font-normal text-amber-700/80 dark:text-amber-300/80 flex items-center gap-1.5 shrink-0 flex-wrap"
					>
						<span>Multitask · {message.spockifyAgents?.status || 'running'}</span>
						{#if agentsHud}
							<span
								class="text-gray-400 dark:text-gray-500"
								title={agentsHud.cost_note || 'rough cost estimate'}
							>
								{#if agentsHud.latency_ms != null}{agentsHud.latency_ms}ms{/if}
								{#if agentsTok}
									{' '}· {agentsTok} tok{/if}
								{#if agentsHud.cost_usd != null}
									{' '}· ~${Number(agentsHud.cost_usd).toFixed(4)}{/if}
							</span>
						{/if}
						{#if agentsRunActive(message.spockifyAgents?.status) && message.spockifyAgents?.id}
							<button
								type="button"
							...
							>
								{agentsCancelBusy ? '…' : 'Cancel'}
							</button>
						{/if}
					</span>
				{/if}

				{#if message.spockifyHud && !(message.model === 'spockify-agents' || message.spockifyAgents)}
					{@const hud = message.spockifyHud}
					{@const tok =
						(hud.total_tokens ?? 0) ||
						(hud.prompt_tokens || 0) + (hud.completion_tokens || 0)}
					<span
						class="text-xs font-normal text-gray-400 dark:text-gray-500 line-clamp-1 shrink-0"
						title={hud.cost_note || 'rough cost estimate'}
					>
				...
				{#if message.spockifyCritique?.level}
					<span
						class="text-xs font-normal line-clamp-1 shrink-0 {message.spockifyCritique.level ===
						'low'
							? 'text-amber-600 dark:text-amber-400'
							: message.spockifyCritique.level === 'high'
								? 'text-emerald-600 dark:text-emerald-400'
								: 'text-gray-400 dark:text-gray-500'}"
						title={message.spockifyCritique.notes || ''}
					>
						confidence: {message.spockifyCritique.level}
					</span>
				{/if}
			...
			<div>
				<div class="chat-{message.role} w-full min-w-full markdown-prose">
					<div>
						...
						{#if message?.files && message.files?.filter( (f) => ['image', 'video', 'file'].includes(f.type) ).length > 0}
							...
							{#if canRequestImageVariation}
								<div class="mt-1 mb-2">
									<Tooltip content="Same prompt, new seed (Flux text2img variation)" placement="top">
										<button
											type="button"
											class="inline-flex items-center gap-1.5 px-2.5 py-1 text-xs rounded-lg border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-800 disabled:opacity-50 transition-colors"
											disabled={variationBusy}
											aria-label="Make another like this"
											on:click|preventDefault={requestImageVariation}
										>
											<Sparkles className="size-3.5" strokeWidth="1.75" />
											{variationBusy
												? $i18n.t('Generating…')
												: $i18n.t('Make another like this')}
										</button>
									</Tooltip>
								</div>
							{:else if canRequestAnotherWebPhoto}
								<div class="mt-1 mb-2">
									<Tooltip content="Fetch another stock/web photo of the same subject" placement="top">
										<button
											type="button"
											class="inline-flex items-center gap-1.5 px-2.5 py-1 text-xs rounded-lg border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-800 disabled:opacity-50 transition-colors"
											disabled={variationBusy}
											aria-label="Show another photo like this"
											on:click|preventDefault={requestAnotherWebPhoto}
										>
											<Sparkles className="size-3.5" strokeWidth="1.75" />
											{variationBusy
												? $i18n.t('Searching…')
				...
				{#if !edit}
					<div
						...
						class="flex justify-start overflow-x-auto scrollbar-none buttons text-gray-600 dark:text-gray-500 mt-0.5 max-w-full"
					>
