...
{#if !status?.hidden}
	<div class="status-description flex items-center gap-2 py-0 w-full text-left text-xs">
				...
				<div class="flex flex-col justify-center -space-y-0.5">
					<div
						class="{(done || status?.done) === false
							? 'shimmer'
							: ''} text-xs line-clamp-1 text-wrap"
					>
		...
		{:else if status?.action === 'knowledge_search'}
			<div class="flex flex-col justify-center -space-y-0.5">
				<div
					class="{(done || status?.done) === false
						? 'shimmer'
						: ''} text-gray-500 dark:text-gray-500 text-xs line-clamp-1 text-wrap"
				>
