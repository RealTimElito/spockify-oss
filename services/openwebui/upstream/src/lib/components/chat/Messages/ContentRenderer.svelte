<script>
	...
	import { shouldAutoOpenCanvas } from '$lib/utils/spockifyCanvas';

	$: floatingActions = (() => {
		const base = $settings?.floatingActionButtons ?? [];
		if (!base.length) return [];
		if (base.some((a) => a?.id === 'ghost' || a?.openGhost)) return base;
		return [...base, { id: 'ghost', label: 'Open in Ghost', openGhost: true }];
	})();
	...
	{#if $settings?.renderMarkdownInAssistantMessages ?? true}
		<Markdown
			...
			onUpdate={async (token) => {
				...
				const wantCanvas =
					($settings?.detectArtifacts ?? true) || $spockifyCanvasEnabled;

				if (
					wantCanvas &&
					shouldAutoOpenCanvas(lang, code, { includeSubstantial: true }) &&
					!$mobile &&
					$chatId
				) {
					await tick();
