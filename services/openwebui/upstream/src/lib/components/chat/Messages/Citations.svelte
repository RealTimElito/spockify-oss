	...
	<div class="py-1.5">
		<div class="text-xs gap-2 flex flex-col">
			{#each citations as citation, idx}
				{@const snippet = (citation.document?.[0] || '').toString().trim()}
				{@const preview =
					snippet.length > 220 ? snippet.slice(0, 217) + '…' : snippet}
				{@const href = citation.source?.url || citation.id || ''}
				<button
					id={`source-${id}-${idx + 1}`}
					aria-label={$i18n.t('View source: {{name}}', {
						name: decodeString(citation.source.name)
					})}
					class="no-toggle outline-hidden flex dark:text-gray-300 bg-transparent text-gray-600 rounded-xl gap-1.5 items-center group/citation"
					title={preview || decodeString(citation.source.name)}
					on:click={() => {
						showCitationModal = true;
						selectedCitation = citation;
					}}
					on:mouseenter={(e) => {
						const el = e.currentTarget.querySelector('[data-cite-preview]');
				...
				>
					...
					<div class="flex-1 min-w-0 relative text-left">
						<div
							class="truncate hover:text-black dark:text-white/60 dark:hover:text-white transition"
						>
						...
						{#if preview}
							<div
								data-cite-preview
								class="hidden absolute z-20 left-0 top-full mt-1 w-72 max-w-[min(18rem,80vw)] rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-2.5 shadow-md text-[11px] leading-snug text-gray-700 dark:text-gray-200"
								role="tooltip"
							>
								<div class="font-medium line-clamp-2 mb-1">
									{decodeString(citation.source.name)}
								</div>
								<div class="line-clamp-4 opacity-90">{preview}</div>
								{#if href && (href.startsWith('http://') || href.startsWith('https://'))}
									<div class="mt-1.5 truncate text-gray-400">{href}</div>
								{/if}
							</div>
						{/if}
					</div>
