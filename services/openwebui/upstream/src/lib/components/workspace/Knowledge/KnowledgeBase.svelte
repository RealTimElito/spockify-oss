			...
			<div class=" flex w-full">
				<div class="flex-1">
					<div class="flex items-center justify-between w-full">
						...
						{#if knowledge?.write_access}
							<div class="self-center shrink-0 flex items-center gap-1">
								<button
									class="bg-gray-50 hover:bg-gray-100 text-black dark:bg-gray-850 dark:hover:bg-gray-800 dark:text-white transition px-2 py-1 rounded-full flex gap-1 items-center text-xs"
									type="button"
									title={$i18n.t('Generate two-voice podcast from knowledge description')}
									on:click={async () => {
										try {
											const { generatePodcast } = await import('$lib/apis/spockify');
											const text = [
												knowledge?.name || '',
												knowledge?.description || '',
												...(Array.isArray(knowledge?.files)
													? knowledge.files
															.slice(0, 5)
															.map((f) => f?.meta?.name || f?.filename || '')
													: [])
											]
												.filter(Boolean)
												.join('\n\n');
											if (!text.trim()) {
												toast.error($i18n.t('No text to podcast'));
												return;
											}
											toast.info($i18n.t('Generating podcast…'));
											const blob = await generatePodcast(localStorage.token, {
												text,
												title: knowledge?.name || 'Spockify podcast'
											});
											const url = URL.createObjectURL(blob);
											const a = document.createElement('a');
											a.href = url;
											a.download = `spockify-podcast-${(knowledge?.name || 'notes').slice(0, 40)}.mp3`;
											a.click();
											URL.revokeObjectURL(url);
											toast.success($i18n.t('Podcast ready'));
										} catch (e) {
											toast.error(`${e}`);
										}
									}}
								>
									{$i18n.t('Generate podcast')}
								</button>
								<button
