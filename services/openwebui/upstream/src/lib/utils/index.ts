...
export const getGravatarURL = (_email) => {
	// Local avatar only — Gravatar requires WAN and breaks air-gapped UIs.
	return '/user.png';
};
...
/**
 * Strip markdown so TTS reads prose, not punctuation names
 * ("asterisk", "hashtag", "underscore", "backtick", …).
 * Used by cleanText → getMessageContentParts (Read aloud + Call).
 */
export const removeFormattings = (str: string) => {
	return (
		str
			// Fenced code: skip heavy blocks; keep short snippets as plain text
			.replace(/```[\w+-]*\r?\n?([\s\S]*?)```/g, (_m, code: string) => {
				const trimmed = (code || '').trim();
				if (!trimmed) return ' ';
				if (trimmed.length > 160 || trimmed.split(/\r?\n/).length > 4) {
					return ' code block. ';
				}
				return ` ${trimmed} `;
			})
			.replace(/```[\w+-]*\r?\n?/g, ' ') // unclosed fence opener
			.replace(/^\|.*\|$/gm, '') // Tables
			.replace(/^\s*\|?[:\s|-]+\|?\s*$/gm, '') // Table separators

			// Images: do not speak URLs/alt markup
			.replace(/!\[([^\]]*)\](?:\([^)]*\)|\[[^\]]*\])/g, ' ')
			// Links: speak label only
			.replace(/\[([^\]]+)\](?:\([^)]*\)|\[[^\]]*\])/g, '$1')
			.replace(/^\[[^\]]+\]:\s*.*$/gm, '') // Reference definitions

			// Emphasis (longer delimiters first); preserve inner text
			.replace(/\*\*\*(.+?)\*\*\*/g, '$1')
			.replace(/___(.+?)___/g, '$1')
			.replace(/\*\*(.+?)\*\*/g, '$1')
			.replace(/__(.+?)__/g, '$1')
			...
			// Math asterisk → readable word before leftover cleanup
			.replace(/(\d)\s*\*\s*(\d)/g, '$1 times $2')
			// Hashtags (keep C# / F# etc.)
			.replace(/(^|[^A-Za-z0-9])#(?=[A-Za-z])/gm, '$1')
			// Word-internal underscores → space (snake_case) before stripping leftovers
			.replace(/(?<=\w)_(?=\w)/g, ' ')

			// Leftover markers voices often pronounce literally
			.replace(/`+/g, '')
			.replace(/[*_]{1,3}/g, '')
			.replace(/^#{1,6}\s*/gm, '')
...
/**
 * On Stop/cancel: mark in-flight reasoning / code_interpreter <details> as done
 * and stamp duration so Collapsible freezes ("Thought for Ns") instead of
 * keeping a live Thinking state.
 */
export const freezeIncompleteDetails = (
	content: string,
	durationSec: number
): string => {
	if (!content || typeof content !== 'string') return content;
	const secs = Math.max(0, Math.floor(Number(durationSec) || 0));
	return content.replace(/<details(\s[^>]*)>/gi, (full, rawAttrs: string) => {
		const attrs = rawAttrs || '';
		if (!/\btype="(reasoning|code_interpreter)"/i.test(attrs)) return full;
		if (/\bdone="true"/i.test(attrs)) return full;

		let next = attrs
			.replace(/\sdone="false"/i, '')
			.replace(/\sduration="[^"]*"/i, '');
		next += ` done="true" duration="${secs}"`;
		return `<details${next}>`;
	});
};
...
export const extractSentencesForAudio = (text: string) => {
	// Merge only fragments that are *both* short (was OR → delayed first TTS a lot).
...
/**
 * Streaming Call TTS: peel the next speakable unit from text not yet spoken.
 * Prefers sentence end, then clause (,;:), then a hard length flush on a word boundary.
 * Returns consumed character count into `text` (including leading whitespace / trailing gap).
 */
export const takeStreamingSpeakableUnit = (
	text: string,
	opts?: { minClauseChars?: number; maxPendingChars?: number }
): { unit: string; consumed: number } | null => {
	if (!text || !text.trim()) return null;
	const minClause = opts?.minClauseChars ?? 40;
	const maxPending = opts?.maxPendingChars ?? 100;

	const sentence = text.match(/^(\s*)([\s\S]+?[.!?])(?=\s|$)/);
	if (sentence) {
		const unit = cleanText(sentence[2]);
		if (unit) {
			const withGap = text.match(/^(\s*)([\s\S]+?[.!?])(\s*)/);
			return { unit, consumed: (withGap ?? sentence)[0].length };
		}
	}

	const newline = text.match(/^(\s*)([^\n]+)(\n+)/);
	if (newline) {
		const unit = cleanText(newline[2]);
		if (unit) {
			return { unit, consumed: newline[0].length };
		}
	}

	const clauseRe = new RegExp(`^(\\s*)([\\s\\S]{${minClause},}?[,;:])(\\s+)`);
	const clause = text.match(clauseRe);
	if (clause) {
		const unit = cleanText(clause[2]);
		if (unit) {
			return { unit, consumed: clause[0].length };
		}
	}

	if (text.trimStart().length >= maxPending) {
		const hard = text.match(/^(\s*)([\s\S]{60,120}?)\s+/);
		if (hard) {
			const unit = cleanText(hard[2]);
			if (unit) {
				return { unit, consumed: hard[0].length };
			}
		}
	}
...
/** Drain all currently ready speakable units from the unspoke suffix. */
export const drainStreamingSpeakableUnits = (
	unspoke: string,
	opts?: { minClauseChars?: number; maxPendingChars?: number }
): { units: string[]; consumed: number } => {
	const units: string[] = [];
	let consumed = 0;
	let rest = unspoke;
	while (true) {
		const next = takeStreamingSpeakableUnit(rest, opts);
		if (!next) break;
		units.push(next.unit);
		consumed += next.consumed;
		rest = unspoke.slice(consumed);
	}
