...
# Nested workers + mid-thought parent handoff (see midthought_spawn.py).
_SPAWN_JSON_RE = re.compile(
    r"(?:SPAWN_CHILDREN|SPAWN)\s*:\s*(\[[\s\S]*?\])",
    re.IGNORECASE,
)
_TERMINAL_STATUSES = frozenset({"done", "failed", "cancelled"})
# Mid-thought opportunistic spawn (not the Heavy 4-role ensemble).
SPAWN_PROFILE = "midthought"
...
def _message_text(result: dict[str, Any]) -> str:
    """Prefer message content; salvage reasoning when content is empty."""
    try:
    ...
    if content:
        return content
    for k in ("reasoning_content", "reasoning", "thinking", "reasoning_text"):
        v = msg.get(k)
...
def _worker_temperature(run: dict[str, Any]) -> float:
    """SPAWN workers slightly cooler for denser DONE digests; Heavy stays 0.4."""
    profile = str(run.get("profile") or "").strip().lower()
    if profile == SPAWN_PROFILE:
        return float(os.getenv("MIDTHOUGHT_WORKER_TEMP", "0.35"))
    return 0.4


def _worker_system_prompt(run: dict[str, Any], worker_name: str, nest_hint: str) -> str:
    profile = str(run.get("profile") or "").strip().lower()
    if profile == SPAWN_PROFILE:
        return (
            f"You are Spockify mid-thought worker '{worker_name}'. "
            "Stay in role. Return COMPLETED WORK, not a plan for someone else.\n"
            "Format:\n"
            "DONE — <one-line outcome>\n"
            "FINDINGS: concrete results (substance, not methodology).\n"
            "ARTIFACTS: key facts, comparisons, or citations if any.\n"
            "Be concise and useful."
            + nest_hint
        )
    return (
        f"You are Spockify parallel agent '{worker_name}'. "
...
def agents_meta_sse(run: dict[str, Any], *, thinking: str = "") -> bytes:
    ...
    Also the Phase A–D mid-thought chip channel: OWUI Thought panel and IDE
    chat should read ``spockify_agents`` from these frames (router SSE parity).
    """
    view = public_run_view(run)
    profile = str(run.get("profile") or "").strip().lower()
    thinking_mode = thinking or (
        "heavy" if profile == "heavy" else ("high" if profile == SPAWN_PROFILE else "")
    )
    if profile == "heavy":
        worker = "heavy"
    elif profile == SPAWN_PROFILE:
        worker = "spawn"
    else:
        worker = "agents"
    ...
    if profile == SPAWN_PROFILE:
        if status == "running":
            desc = f"Spawn · {done_workers}/{total_workers}…"
    ...
    *,
    ...
    async with sem:
        ...
        messages = [
            {
                "role": "system",
                "content": _worker_system_prompt(run, worker["name"], nest_hint),
            },
            {"role": "user", "content": user_content},
        ]
        ...
        try:
            chat_kwargs: dict[str, Any] = {
                "temperature": _worker_temperature(run),
...
async def _synthesize(
    *,
    ...
    except Exception:  # noqa: BLE001 — synth best-effort
        return ""
...
async def execute_run(
    ...
    *,
    ...
    try:
        ...
        sem = asyncio.Semaphore(
            int(run.get("midthought_cap") or run.get("max_workers") or AGENTS_MAX_WORKERS)
        )
