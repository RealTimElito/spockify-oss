...
class CoderWorkerPromptTests(unittest.TestCase):
    ...
    def test_single_hyphen_tag_still_maps(self) -> None:
        ...
        self.assertEqual(_to_ollama_model("llama3.2-3b-cpu"), "llama3.2-3b-cpu")
        self.assertEqual(_to_ollama_model("llama"), "llama3.2:3b")
        ...
        self.assertEqual(_to_ollama_model("devstral-2"), "devstral-2")
        self.assertEqual(_to_ollama_model("ministral-3-14b"), "ministral-3:14b")

    def test_bare_llama_orchestrator_canonicalizes(self) -> None:
        decision = _parse_routing(
            '{"selected_model": "llama", "needs_web_search": false, '
            '"task_type": "casual_chat", "confidence": 0.9}'
        )
        self.assertEqual(decision.selected_model, "llama3.2-3b")
        self.assertEqual(_to_ollama_model(decision.selected_model), "llama3.2:3b")
        self.assertEqual(
            model_catalog.canonicalize_worker("llama"), "llama3.2-3b"
        )
...
class GemmaThinkingTests(unittest.TestCase):
    def test_ollama_supports_thinking_families(self) -> None:
        ...
        self.assertFalse(_ollama_supports_thinking("devstral-2"))
...
class UncertaintyRsiTests(unittest.TestCase):
    ...
    def test_demo_system_prompt_empty_without_profile(self) -> None:
        import main

        main.SPOCKIFY_DEMO_PROFILE = ""
        main._DEMO_SYSTEM_PROMPT_CACHE = None
        self.assertEqual(main._demo_system_prompt(), "")

    def test_demo_system_prompt_loads_authorized_file(self) -> None:
        import tempfile
        ...
        text = (
            ...
            "Prefer concrete defensive controls.\n"
        )
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp) / "demo.md"
            path.write_text(text, encoding="utf-8")
            main.SPOCKIFY_DEMO_PROFILE = "security"
            main.DEMO_SYSTEM_PROMPT_PATH = str(path)
            main._DEMO_SYSTEM_PROMPT_CACHE = None
            loaded = main._demo_system_prompt()
            self.assertIn("Authorized security", loaded)
            self.assertIn("defensive", loaded.lower())
            self.assertNotIn("no morals", loaded.lower())
            self.assertNotIn("jailbreak", loaded.lower())
            routed = main._routing_system_prompt()
            self.assertIn("Demo profile (security)", routed)
            self.assertIn("Authorized security", routed)
...
class ModelCatalogTests(unittest.TestCase):
    ...
    def test_mistral_new_models_think_and_map(self) -> None:
        ...
        self.assertEqual(model_catalog.thinking_api_kind("devstral-2"), "none")
        self.assertIsNone(model_catalog.ollama_think_value("devstral-2", "high"))
        ...
        self.assertEqual(model_catalog.get_model("devstral-2").vram_class, "xlarge")
        self.assertIn("code", model_catalog.get_model("devstral-2").strengths)
        self.assertEqual(model_catalog.get_model("devstral-2:123b").alias, "devstral-2")
        self.assertEqual(
            model_catalog.resolve_explicit_model_request("talk to Devstral 2"),
            "devstral-2",
        )
        self.assertEqual(
            model_catalog.resolve_explicit_model_request("use devstral-small-2"),
            "devstral-small-2",
        )
