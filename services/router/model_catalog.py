"""Single source of truth for local Spark chat models.

Imported by the router, parallel-agent planner, and tests. Keep aliases in
sync with OLLAMA_MODEL_MAP / pull-models.sh Spark + Qwen sets.
"""
...
# Local chat models actually pulled on Spark (plus web-* wrappers).
...
_MODELS: tuple[ChatModel, ...] = (
    ...
    ChatModel(
        ...
        ollama_name="llama3.2-3b-cpu",
        family="llama",
        vram_class="tiny",
        strengths=("speed", "greetings", "acks"),
        supports_thinking=False,
        notes="CPU-only alias (num_gpu 0) for OSS/laptop compose. No think=.",
    ),
    ChatModel(
        alias="llama3.1-8b",
    ...
    ChatModel(
        alias="devstral-2",
        ollama_name="devstral-2",
        family="mistral",
        vram_class="xlarge",
        strengths=("code", "agentic"),
        supports_thinking=False,
        notes=(
            "Devstral 2 123B agentic coder (~75GB, on-demand; Spark solo). "
            "No think=. Opt-in only — not the default English code route "
            "(gpt-oss-120b)."
        ),
    ),
    ChatModel(
...
_ALIAS_REDIRECT: dict[str, str] = {
    ...
    # Orchestrator sometimes emits family-only "llama" (not a pulled tag).
    "llama": "llama3.2-3b",
...
_EXPLICIT_FAMILY_SPECS: tuple[tuple[str, str], ...] = (
    ...
    (r"devstral[\s\-]?2(?![\s\-]?small)|devstral[\s\-]?123b", "devstral-2"),
...
def canonicalize_worker(name: str, fallback: str = "llama3.2-3b") -> str:
    """Map orchestrator/user shorthand to a catalog alias.

    Bare family names like ``llama`` are not Ollama tags; remap them so
    the router never calls ``/api/chat`` with a missing model id.
    """
    row = get_model(name)
    if row is not None:
        return row.alias
    fb = get_model(fallback)
    if fb is not None:
        return fb.alias
    return (fallback or "llama3.2-3b").strip() or "llama3.2-3b"
...
def orchestrator_catalog_text() -> str:
    ...
    lines.append(
        ...
        "(llama3.* / codestral / devstral / ministral / mathstral / phi / "
        "llava). Never emit bare selected_model llama."
    )
    return "\n".join(lines)
    ...
    *,
    ...
    if mode == "off":
        return (
            ...
            "worker including llama3.2-3b / llama3.1-8b / codestral / "
            "mathstral (never the bare id llama). Prefer fast/cheap "
    ...
    if mode == "low":
        return (
            ...
            "llama3.2-3b / llama3.1-8b / codestral are fine (never bare "
            "llama). Remap qwen3.6-* to qwen3.5-9b. "
    ...
    if mode == "high":
        return (
            ...
            "Do not pick llama3.2-3b / llama3.1-8b for "
    ...
    return (
        ...
        "llama3.2-3b / llama3.1-8b / codestral / devstral are "
