...
import midthought_spawn as mtspawn
...
# Optional authorized demo overlay (e.g. security). Never jailbreak / unfiltered.
SPOCKIFY_DEMO_PROFILE = (os.getenv("SPOCKIFY_DEMO_PROFILE") or "").strip().lower()
DEMO_SYSTEM_PROMPT_PATH = os.getenv(
    "DEMO_SYSTEM_PROMPT_PATH", "/config/demo-system-prompt.md"
)
...
OLLAMA_MODEL_MAP: dict[str, str] = {
    ...
    # Family-only shorthand from the orchestrator — not a real Ollama tag.
    "llama": "llama3.2:3b",
    "llama3.2-3b": "llama3.2:3b",
    # Identity map — rpartition would wrongly yield llama3.2-3b:cpu.
    "llama3.2-3b-cpu": "llama3.2-3b-cpu",
    ...
    # Official tags are hyphenated; naive rpartition would yield
...
def _routing_system_prompt(
    *,
    ...
    out = f"{base}\n\n{catalog}\n\n{policy}"
    demo = _demo_system_prompt()
    if demo:
        out = (
            f"{out}\n\n## Demo profile ({SPOCKIFY_DEMO_PROFILE})\n\n{demo}"
        )
    return out
...
def _load_demo_system_prompt() -> str:
    """Authorized demo guidance only; empty unless SPOCKIFY_DEMO_PROFILE is set."""
    if not SPOCKIFY_DEMO_PROFILE:
        return ""
    try:
        with open(DEMO_SYSTEM_PROMPT_PATH, encoding="utf-8") as f:
            return f.read().strip()
    except OSError:
        return ""


_DEMO_SYSTEM_PROMPT_CACHE: Optional[str] = None


def _demo_system_prompt() -> str:
    global _DEMO_SYSTEM_PROMPT_CACHE
    if _DEMO_SYSTEM_PROMPT_CACHE is None:
        _DEMO_SYSTEM_PROMPT_CACHE = _load_demo_system_prompt()
    return _DEMO_SYSTEM_PROMPT_CACHE
...
def _parse_routing(raw: str) -> RoutingDecision:
    try:
        ...
        worker = model_catalog.canonicalize_worker(
            str(worker or ""), fallback=DEFAULT_CHAT_WORKER
        )
        return RoutingDecision(
...
async def _build_worker_messages(
    ...
    *,
    ...
    demo = _demo_system_prompt()
    if demo:
        prefix.append({"role": "system", "content": demo})
    sources: list[dict[str, Any]] = []
    if decision.needs_web_search:
...
def _read_meminfo_bytes() -> dict[str, Any]:
    ...
    return {
        "ok": True,
        "source": "proc_meminfo",
        "note": "Pod/cgroup view; GB10 unified memory may differ on the host.",
        "total_bytes": total,
...
async def spockify_status() -> dict[str, Any]:
    """Read-only Spark health: Ollama loaded models, ComfyUI, free RAM/GPU."""
    async with httpx.AsyncClient() as client:
    ...
    gpu: dict[str, Any] = {
        "source": None,
        "devices": [],
        "note": "GB10 uses unified memory; prefer ComfyUI device stats when up.",
    }
    ...
    return {
        ...
        "agents": {
            ...
            "midthought_spawn": mtspawn.public_spawn_limits(),
        },
        "browser": {
...
def _sse_delta_fields(chunk: bytes) -> tuple[str, str]:
    """Extract (content, reasoning_content) from one OpenAI-style SSE frame."""
    content = ""
    reasoning = ""
    try:
        if not chunk.startswith(b"data:"):
...
def _reasoning_sse_delta(text: str, *, model: str = SPOCKIFY_DISPLAY_MODEL) -> bytes:
    """Keep Thought panel live during buffered parent content / spawn wait."""
    if not text:
        return b""
    chunk = {
        "id": f"chatcmpl-{uuid.uuid4().hex[:24]}",
...
async def _execute_midthought_spawn_round(
    *,
    req: ChatCompletionRequest,
    user_msg: str,
    parent_segment: str,
    ...
    """Launch children via run store, emit chips, same-parent merge stream."""
    specs = mtspawn.parse_parent_spawn(parent_segment)
    if not mtspawn.spawn_args_clear(specs):
        # Weak SPAWN — treat as normal answer (stripped).
        cleaned = mtspawn.strip_spawn_marker(parent_segment)
        ask = pagents.extract_ask_user(parent_segment)
        if ask:
            cleaned = pagents.ensure_questions_visible(cleaned, ask)
        step = 240
        for i in range(0, len(cleaned), step):
            yield pagents.content_sse_delta(
                cleaned[i : i + step], model=SPOCKIFY_DISPLAY_MODEL
            )
        return

    n = len(specs)
    yield _status_sse(
        f"Spawn · launching {n} worker{'s' if n != 1 else ''}…",
        done=False,
        worker="spawn",
    )
    # Surface plan notes into Thought so UI stays alive before chips update.
    plan = mtspawn.strip_spawn_marker(parent_segment).strip()
    if plan:
        note = plan if len(plan) <= 1200 else plan[:1199] + "…"
        yield _reasoning_sse_delta(
            f"\n[Spawn plan]\n{note}\n", model=SPOCKIFY_DISPLAY_MODEL
        )
    ...
    body = pagents.AgentRunCreate(
        ...
        model=worker,
        workers=specs,
        synthesize=False,  # same-parent merge below — not a third synth model
        parent_chat_id=getattr(req, "spockify_chat_id", None),
        parent_message_id=getattr(req, "spockify_message_id", None),
        user_id=getattr(req, "spockify_user_id", None),
        profile=pagents.SPAWN_PROFILE,
        worker_timeout=float(os.getenv("MIDTHOUGHT_WORKER_TIMEOUT", "180")),
        max_tokens=int(os.getenv("MIDTHOUGHT_WORKER_MAX_TOKENS", "2048")),
        think="medium" if thinking_mode == "high" else False,
        tools=["search", "browse"],
    )
    run = pagents.create_run_record(body)
    # Cap concurrent workers to mid-thought profile (Spark 3 / compose 2).
    cap = mtspawn.spawn_cap()
    # Temporarily bound semaphore via AGENTS_MAX_WORKERS semantics inside execute:
    # create_run already capped specs; also shrink env-facing note on run.
    run["midthought_cap"] = cap
    run_id = str(run["id"])
    yield pagents.agents_meta_sse(run, thinking=thinking_mode)

    async def _run_job() -> None:
        async with httpx.AsyncClient() as client:
            # Narrow concurrency for this run without mutating global forever:
            # execute_run uses AGENTS_MAX_WORKERS semaphore; pass via monkeypatch
            # of run-local is not supported — set workers already ≤ cap.
            await pagents.execute_run(
                run,
                client=client,
                worker_chat=_worker_chat,
            ...
            if changed or due:
                ...
                if changed:
                    done_n = sum(
                        1
                        for w in (current.get("workers") or [])
                        if w.get("status") in ("done", "failed", "cancelled")
                    )
                    total_n = len(current.get("workers") or [])
                    yield _status_sse(
                        f"Spawn · {done_n}/{total_n}…",
                        done=False,
                        worker="spawn",
                    )
                else:
                    yield b": spawn-keepalive\n\n"
                yield pagents.agents_meta_sse(current, thinking=thinking_mode)
            await asyncio.sleep(0.35)
        await task
    except asyncio.CancelledError:
        LOG.info("midthought stream disconnected; run %s continues", run_id)
        return

    final = pagents.get_run(run_id) or run
    yield pagents.agents_meta_sse(final, thinking=thinking_mode)
    digests = mtspawn.format_done_digests(final)
    questions = pagents.collect_run_questions(final)

    yield _status_sse("Spawn · merging…", done=False, worker="spawn")
    merge_messages = mtspawn.build_merge_messages(
        user_msg=user_msg,
        parent_segment=parent_segment,
        digests=digests,
        prior_messages=worker_messages,
    )

    # Same parent model (quality + warm weights). Escape hatch: lighter model
    # only if memory_guard refuses — otherwise keep parent.
    merge_model = worker
    try:
        import memory_guard as memguard

        async with httpx.AsyncClient() as guard_client:
            allowed, reason, _snap = await memguard.heavy_mode_allowed(guard_client)
        if not allowed and worker in (
            "gpt-oss-120b",
            "gpt-oss:120b",
            "gemma4-31b",
            "devstral-2",
            ...
            LOG.warning(
                "midthought merge downgrade %s → gpt-oss-20b (%s)", worker, reason
            )
            merge_model = "gpt-oss-20b"
    except Exception as exc:  # noqa: BLE001
        LOG.debug("midthought memory_guard skip: %s", exc)
    ...
    async for chunk in _worker_chat_stream(
        ...
        if chunk.startswith(b"data:"):
            ...
            if raw == b"[DONE]":
                continue
        piece, reasoning = _sse_delta_fields(chunk)
        if reasoning:
            yield _reasoning_sse_delta(reasoning, model=SPOCKIFY_DISPLAY_MODEL)
        if piece:
            collected.append(piece)
            yield pagents.content_sse_delta(piece, model=SPOCKIFY_DISPLAY_MODEL)

    answer = mtspawn.strip_spawn_marker("".join(collected))
    if questions:
        patched = pagents.ensure_questions_visible(answer, questions)
        if patched != answer:
            yield pagents.content_sse_delta(
                patched[len(answer) :], model=SPOCKIFY_DISPLAY_MODEL
            )
    ...
    async with httpx.AsyncClient() as client:
        ...
        if req.stream:
            ...
            async def event_stream() -> AsyncIterator[bytes]:
                ...
                spawn_ok = mtspawn.spawn_eligible(
                    thinking_mode,
                    voice_mode=voice_mode,
                    trivial=trivial_turn or _is_trivial_worker_turn(user_msg, decision),
                    explicit_alias=bool(explicit_alias),
                    pipeline=False,
                )
                try:
                    status = _routing_status_message(decision, user_msg)
                    ...
                    parent_messages = worker_messages
                    if spawn_ok:
                        parent_messages = mtspawn.inject_spawn_hint(
                            worker_messages, thinking_mode
                        )
                    ...
                    async for chunk in _stream_worker_with_preamble(
                        ...
                        **({"stop": call_stop} if call_stop else {}),
                        ...
                        try:
                                ...
                                if raw:
                                    ...
                                    if isinstance(data, dict):
                                        ...
                                        for ch in data.get("choices") or []:
                                            ...
                                            if spawn_ok and piece:
                                                content_buf.append(piece)
                                                # Phase B: segment break once SPAWN JSON is complete.
                                                if mtspawn.spawn_marker_complete(
                                                    "".join(content_buf)
                                                ):
                                                    spawn_break = True
                                                # Buffer content — stream reasoning only until we know.
                                                if reasoning:
                                                    yield _reasoning_sse_delta(
                                                        reasoning,
                                                        model=SPOCKIFY_DISPLAY_MODEL,
                            ...
                            pass
                        if spawn_ok:
                            # Forward non-content frames (status/attribution/reasoning-only).
                            _c, _r = _sse_delta_fields(chunk)
                            if _c and not _r:
                                continue
                            if _c and _r:
                                # content already buffered; emit reasoning half only
                                yield _reasoning_sse_delta(
                                    _r, model=SPOCKIFY_DISPLAY_MODEL
                                )
                                if spawn_break:
                                    break
                                continue
                    ...
                    parent_text = "".join(content_buf) if spawn_ok else "".join(collected)
                    did_spawn = False
                    if spawn_ok:
                        specs = mtspawn.parse_parent_spawn(parent_text)
                        if mtspawn.spawn_args_clear(specs):
                            did_spawn = True
                            LOG.info(
                                "midthought SPAWN n=%s mode=%s break=%s",
                                len(specs),
                                thinking_mode,
                                spawn_break,
                            )
                            collected = []
                            async for frame in _execute_midthought_spawn_round(
                                req=req,
                                user_msg=user_msg,
                                parent_segment=parent_text,
                                worker=worker,
                                thinking_mode=thinking_mode,
                                ...
                                # Collect merge content for HUD/critique.
                                piece = _delta_content_from_sse_chunk(frame)
                                if piece:
                                    collected.append(piece)
                                yield frame
                        else:
                            # No clear SPAWN — flush buffered parent content.
                            flush = mtspawn.strip_spawn_marker(parent_text)
                            ask = pagents.extract_ask_user(parent_text)
                            if ask:
                                flush = pagents.ensure_questions_visible(flush, ask)
                            step = 240
                            for i in range(0, len(flush), step):
                                yield pagents.content_sse_delta(
                                    flush[i : i + step],
                                    model=SPOCKIFY_DISPLAY_MODEL,
                                )
