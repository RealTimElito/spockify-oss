...
function ensurePreviewStatusBar(): vscode.StatusBarItem {
  if (!previewStatus) {
...
function showGeneratingStatusBar(): void {
  const item = ensurePreviewStatusBar();
  item.text = '$(loading~spin) Inline: Generating…';
  item.tooltip = 'Spockify Ctrl+K streaming preview';
...
function showPreviewStatusBar(session: InlineEditSession): void {
  const item = ensurePreviewStatusBar();
  item.text = '$(edit) Inline: Accept Ctrl+Enter · Reject Esc';
  item.tooltip = 'Spockify Ctrl+K preview active';
  item.command = CMD.accept;
  item.show();
  void session;
}
...
async function startInlineEdit(
  ...
  ui.setStreaming();
  showGeneratingStatusBar();

  await vscode.window.withProgress(
    {
      location: vscode.ProgressLocation.Notification,
      title: 'Spockify inline edit (Generating…)…',
      cancellable: true,
    },
