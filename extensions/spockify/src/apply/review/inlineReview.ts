...
function refreshVisibleEditors(): void {
  ...
  void vscode.commands.executeCommand(
    'setContext',
    'spockify.inlineReview.hasPending',
    reviews.size > 0,
  );
}

function activeReviewPath(): string | undefined {
  const ed = vscode.window.activeTextEditor;
  if (!ed) return undefined;
  const review = findReview(ed.document.uri);
  if (review) return review.path;
  // Fall back to first pending path so keybindings still work from chat focus.
  return [...reviews.values()][0]?.path;
}
...
export function registerInlineFileReview(
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand(
      ...
      async (path?: string) => {
        const p = typeof path === 'string' && path ? path : activeReviewPath();
        if (p) await acceptFile(p);
      },
    ),
    vscode.commands.registerCommand(
