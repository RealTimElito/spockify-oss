/** ARCHIVED — not imported. Live path: ../agentLoop.ts → @spockify/harness runHarness. */
/**
        ...
        try {
          for await (const chunk of this.deps.transport.streamChatCompletions(
            ...
            if (chunk.spockifyAgents) {
              emit(opts, { type: 'agents', run: chunk.spockifyAgents });
            }
            if (chunk.model) {
 ...
 * Phase 4: renamed from runAgentTurn so @spockify/harness owns that symbol.
 * IDE runtime remains AgentRuntime until full harness ModelTransport wire-up.
 */
