  ...
  private async runViaAgentRuntime(
    ...
    const result = await runtime.run({
      ...
      onEvent: (ev) => {
        ...
        if (ev.type === 'model' && ev.model) {
        ...
        } else if (ev.type === 'agents' && ev.run) {
          // Mid-thought SPAWN / Heavy chips via router SSE — not a second orchestrator.
          void import('../agents/agentRunChatBridge')
            .then(({ agentRunToCardPayload, publishAgentRunToChat }) => {
              const card = agentRunToCardPayload(
                ev.run as Parameters<typeof agentRunToCardPayload>[0],
              );
              if (card) publishAgentRunToChat(card);
            })
            .catch(() => undefined);
          const status = String(
            (ev.run as { status?: string }).status || 'running',
          );
          const workers = Array.isArray(
            (ev.run as { workers?: unknown[] }).workers,
          )
            ? ((ev.run as { workers: unknown[] }).workers as unknown[])
            : [];
          const doneN = workers.filter((w) => {
            ...
            return /^(done|failed|cancelled)$/i.test(st);
          }).length;
          const total = workers.length;
          const profile = String(
            (ev.run as { profile?: string }).profile || '',
          ).toLowerCase();
          const spawnish =
            profile === 'midthought' ||
            profile === 'spawn' ||
          ...
          if (status === 'synthesizing') {
            activity = spawnish ? 'Merging…' : 'Synthesizing…';
          } else if (total > 0) {
            activity = spawnish
              ? `Spawn · ${doneN}/${total}`
              : `Agents · ${doneN}/${total}`;
          } else {
            activity = spawnish ? `Spawn · ${status}` : `Agents · ${status}`;
          }
          getSessionManager().setActivityLabel(run.agentSessionId, activity);
          this.emit(tabId, {
            type: 'status',
            text: activity,
            chatTabId: tabId,
