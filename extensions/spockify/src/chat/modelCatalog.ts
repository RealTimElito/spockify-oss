...
/** Prod coding pins — keep in sync with @spockify/harness promotedCodingIds. */
const FALLBACK_CODING_PINS = ['gpt-oss-20b', 'gpt-oss-120b'] as const;

function codingPins(): string[] {
  try {
    // Lazy require so a packaged AppImage missing this dep still activates.
    // eslint-disable-next-line @typescript-eslint/no-require-imports
    const h = require('@spockify/harness') as {
      promotedCodingIds?: () => string[];
    };
    const ids = h.promotedCodingIds?.();
    if (Array.isArray(ids) && ids.length) return ids;
  } catch {
    // bundled inject historically omitted @spockify/harness
  }
  return [...FALLBACK_CODING_PINS];
}

const FALLBACK_LAB_ROWS = [
  { id: 'devstral-small-2', label: 'Devstral Small 2' },
  { id: 'devstral-2', label: 'Devstral 2 (evicts 120b-hot)' },
  { id: 'qwen3-coder-30b-a3b', label: 'Qwen3 Coder 30B A3B (Instruct Q4/Q8)' },
] as const;

function labPickerRows(): Array<{ id: string; label: string }> {
  try {
    // eslint-disable-next-line @typescript-eslint/no-require-imports
    const h = require('@spockify/harness') as {
      unpublishedLabModels?: () => Array<{ id: string }>;
    };
    const rows = h.unpublishedLabModels?.();
    if (Array.isArray(rows) && rows.length) {
      return rows.map((r) => {
        const fallback = FALLBACK_LAB_ROWS.find((f) => f.id === r.id);
        return { id: r.id, label: fallback?.label || r.id };
      });
    }
  } catch {
    // same as codingPins — packaged IDE may omit harness
  }
  return [...FALLBACK_LAB_ROWS];
}
...
export const CATALOG_MODELS: readonly CatalogModel[] = [
  ...
  {
    alias: 'devstral-2',
    label: 'Devstral 2 (evicts 120b-hot)',
    family: 'mistral',
    thinkingApi: 'none',
  ...
  },
  {
    ...
    label: 'Qwen3 Coder 30B A3B (Instruct Q4/Q8)',
    family: 'qwen',
    thinkingApi: 'effort',
    strengths: ['code'],
  },
...
export function mergePickerModels(
  ...
  const labRows = labPickerRows();
  const labIds = new Set(labRows.map((r) => r.id.toLowerCase()));
  if (codingOnly) {
    const pins = new Set(codingPins().map((p) => p.toLowerCase()));
    const filtered = out.filter((m) => {
      const n = m.id.toLowerCase().replace(/:/g, '-');
      return m.id === AUTO_MODEL_ID || pins.has(n) || labIds.has(n);
    });
    for (const row of labRows) {
      if (!filtered.some((m) => m.id.toLowerCase() === row.id.toLowerCase())) {
        filtered.push({
          id: row.id,
          label: row.label,
          family: getCatalogModel(row.id)?.family,
          oss: true,
        });
      }
    }
    return filtered.map((m) => {
      const n = m.id.toLowerCase().replace(/:/g, '-');
      const section: PickerModel['section'] =
        m.id === AUTO_MODEL_ID ? 'auto' : labIds.has(n) ? 'lab' : 'prod';
      const label =
        n === 'devstral-2' && !/evict/i.test(m.label)
          ? `${m.label} (evicts 120b-hot)`
          : m.label;
      return { ...m, label, section };
    });
  }
  return out;
}
