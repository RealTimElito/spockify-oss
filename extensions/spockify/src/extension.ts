...
import { registerLabAgents } from './lab/register';
import {
  registerChatPanel,
...
export function activate(context: vscode.ExtensionContext): void {
  ...
  // Chat commands first so palette entries work even if later setup throws.
  let chatProvider: ReturnType<typeof registerChatPanel>['provider'] | undefined;
  registerChatCommand(context, getTransport, output, () => chatProvider);

  try {
    activateRest(context, {
      output,
      getTransport,
      getTransportQuiet,
      refreshAuthUi,
      setChatAuth: (signedIn, label) => {
        setChatAuth(signedIn, label);
      },
      setChatProvider: (p) => {
        chatProvider = p;
        setChatAuth = (signedIn, label) => p.setAuth(signedIn, label);
      },
    });
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    output.appendLine(`activation continued after error: ${msg}`);
    void vscode.window.showWarningMessage(
      `Spockify Chat is available, but some features failed to start: ${msg}`,
    );
  }
}
...
function activateRest(
  ...
  deps: {
    ...
    getTransportQuiet: () => Promise<ModelTransport | undefined>;
    refreshAuthUi: () => Promise<void>;
    setChatAuth: (signedIn: boolean, label?: string) => void;
    setChatProvider: (
      p: ReturnType<typeof registerChatPanel>['provider'],
    ) => void;
  },
): void {
  const { output, getTransport, getTransportQuiet, refreshAuthUi } = deps;
  let setChatAuth = deps.setChatAuth;
  ...
  deps.setChatProvider(chat.provider);
  ...
  registerLabAgents(context, output);
