...
(function () {
  ...
  function renderModelList(filter) {
    ...
    rows.forEach(function (m, idx) {
      if (m.section && m.section !== lastSection) {
        ...
        if (m.section === 'lab' || m.section === 'prod') {
          const hdr = document.createElement('div');
          hdr.className = 'model-section';
          hdr.textContent = m.section === 'lab' ? 'Lab models' : 'Coding pins';
          el.modelList.appendChild(hdr);
        }
      }
      const btn = document.createElement('button');
      btn.type = 'button';
  ...
  function setFilesChanged(count) {
    ...
    if (n > 0 && !streaming) {
      ...
      if (el.streamPhaseLabel && el.streamPhaseBar) {
        el.streamPhaseBar.hidden = false;
        el.streamPhaseBar.dataset.phase = 'review';
        el.streamPhaseLabel.textContent =
          n +
          (n === 1 ? ' file pending' : ' files pending') +
          ' · Ctrl/Cmd+Shift+Enter Accept all';
      }
      if (el.composerHint) {
        el.composerHint.textContent =
          'Review file changes · Keep Ctrl/Cmd+Shift+Y · Undo Ctrl/Cmd+Shift+U';
      }
    } else if (n <= 0 && streamPhase === 'review') {
      setStreamPhase(streaming ? 'thinking' : 'idle');
      if (el.composerHint && !streaming) {
        el.composerHint.textContent =
  ...
  function renderAgentsActivityBar(payload) {
    ...
    if (el.agentsActivityTitle) {
      const spawnish =
        status === 'synthesizing' ||
        /spawn/i.test(String(payload.description || '')) ||
        /midthought|spawn/i.test(String(payload.profile || ''));
      const noun = spawnish ? 'Spawn' : 'Agents';
      if (status === 'synthesizing') {
        el.agentsActivityTitle.textContent = spawnish
          ? 'Spawn · merging…'
          : 'Agents · synthesizing…';
      } else {
        el.agentsActivityTitle.textContent = active
          ? noun + ' · ' + doneN + '/' + (workers.length || '?')
          : noun + ' · ' + status;
      }
    }
  ...
  function setStreaming(on) {
    ...
    if (el.composerHint) {
      ...
      } else if (el.filesChangedBar && !el.filesChangedBar.hidden) {
        el.composerHint.textContent =
          'Review file changes · Keep Ctrl/Cmd+Shift+Y · Undo Ctrl/Cmd+Shift+U';
      } else {
        el.composerHint.textContent =
  ...
  /** After ≥1 tool this turn, idle copy is “Planning next moves” (Cursor-like). */
  let toolsSeenThisStream = 0;
  ...
  function liveThoughtVerb() {
    if (streamPhase === 'wrapping') return 'Wrapping up…';
    if (toolsSeenThisStream > 0) return 'Planning next moves…';
    return 'Thinking…';
  }
  ...
  function paintLiveThought() {
    ...
    liveThought.node.textContent = liveThoughtVerb() + ' ' + secs + 's';
  ...
  function startLiveThought() {
    ...
    line.textContent = liveThoughtVerb() + ' 0s';
  ...
  window.addEventListener('message', function (event) {
    ...
    switch (msg.type) {
      ...
      case 'toolResult':
        ...
        if (streaming) setStreamPhase('planning');
      ...
      case 'streamDone':
        ...
        if (streaming) {
          if (toolsSeenThisStream > 0) {
            setStreamPhase('wrapping');
          }
        ...
        hideToolConsent();
        // Clear mid-thought / Heavy agents chrome when the answer is done.
        renderAgentsActivityBar(null);
      ...
      case 'status':
        if (!routesToActiveTab(msg)) break;
        // Mid-thought SPAWN / merge status → phase bar (Cursor-like live HUD).
        if (typeof msg.text === 'string' && msg.text) {
          const t = msg.text;
          if (/merging/i.test(t)) {
            setStreamPhase('merging');
          } else if (/spawn/i.test(t)) {
            setStreamPhase('spawn');
          } else if (/planning next/i.test(t)) {
            setStreamPhase('planning');
          }
          if (el.streamPhaseLabel && /spawn|merg|agent/i.test(t)) {
            el.streamPhaseLabel.textContent = t;
            if (el.streamPhaseBar) el.streamPhaseBar.hidden = false;
          }
        }
        break;
      case 'queuedSends':
