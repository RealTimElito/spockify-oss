...
describe('modelCatalog', () => {
  it('lists Spark chat aliases (no Kimi / MiMo)', () => {
    ...
    assert.ok(aliases.includes('qwen3-coder-30b-a3b'));
  ...
  it('merges coding catalog by default (IDE picker)', () => {
    ...
    assert.ok(ids.includes(AUTO_MODEL_ID));
    assert.ok(ids.includes('devstral-small-2'));
    assert.ok(ids.includes('devstral-2'));
    assert.ok(ids.includes('qwen3-coder-30b-a3b'));
    ...
    assert.equal(merged.find((m) => m.id === 'devstral-2')?.section, 'lab');
    assert.equal(merged.find((m) => m.id === 'gpt-oss-20b')?.section, 'prod');
  });
