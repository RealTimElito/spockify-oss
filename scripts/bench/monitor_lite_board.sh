...
PY
  ...
  alive=0
  # Prefer exact mini-swe worker; also treat launcher as alive to avoid
  # double-start races while run-swebench.sh is still installing/configuring.
  if pgrep -f 'python -m run_mini_swe swebench' >/dev/null \
    || pgrep -f 'scripts/bench/run_lite_board.sh' >/dev/null; then
    alive=1
  fi
  n_mini="$(pgrep -fc 'python -m run_mini_swe swebench' 2>/dev/null || echo 0)"
  cid="$(docker ps --filter name=minisweagent -q | head -1 || true)"
  free_g="$(df -BG / | awk 'NR==2 {gsub(/G/,"",$4); print $4}')"
  echo "$ts alive=$alive mini=$n_mini preds=$preds nonempty=$nonempty bad=$bad container=${cid:-none} free=${free_g}G" | tee -a "$LOG"

  # Auto-restart if dead and incomplete (flock avoids duplicate launchers).
  ...
  # Drop empty/bogus patches so resume retries them
  if [[ "$bad" -gt 0 ]]; then
    python3 - <<PY
import json
from pathlib import Path
...
PY
  ...
  # Keep Spark VRAM for gpt-oss pins — stop other loaded Ollama tags only.
