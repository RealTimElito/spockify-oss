...
/** Compact upstream JSON/text for 4xx/5xx — CLI was swallowing the body. */
export function formatApiErrorDetail(body: string, max = 500): string {
  const raw = (body || '').trim();
  if (!raw) return '';
  try {
    ...
    const nested =
      typeof parsed.error === 'object' && parsed.error
        ...
        : typeof parsed.error === 'string'
          ? parsed.error
          : undefined;
    const msg = nested || parsed.message || parsed.detail;
    if (typeof msg === 'string' && msg.trim()) return msg.trim().slice(0, max);
...
export function formatApiErrorMessage(
  ...
  const head = `Spockify API ${status} ${statusText} for ${url}`;
  return detail ? `${head}\n${detail}` : head;
}
  ...
  async request<T>(path: string, init: RequestInit = {}): Promise<T> {
    ...
    if (!res.ok) {
      const err = new SpockifyHttpError(
        formatApiErrorMessage(res.status, res.statusText, url, text),
        res.status,
        text,
      );
