...
| Dense | Vectors in index (`hash-local` offline, or remote `nomic-embed` via ide-client → spockify.eu → Spark) |
...
**Cloud vector sync:** deferred — Spark only stores **index metadata** (`/api/v1/spockify/ide/index`). Uploading chunk text / embedding matrices overnight was judged too risky for web (size, storage, RollingUpdate blast radius). Local Lance + nomic remains source of truth.

Full-tree smoke: `npx tsx scripts/reindex-tree.ts /path/to/agentHub` (respects `.gitignore` / `.spockifyignore`).

Live Spark embed proof: `SPOCKIFY_API_KEY=… npm test -- test/live-nomic-hybrid.test.ts` (skips when unset).
...
That is the documented semantic win for Phase 3 DoD (local/controlled embed). Production path uses Spark `nomic-embed` when auth + `/v1/embeddings` succeed; otherwise hash-vector fallback (still hybrid fused, weaker semantics).
...
- Remote embed calls still go **local IDE → spockify.eu → Spark** (not via the SSH host).
