...
export function listHarnesses(projectRoot?: string): HarnessYaml[] {
  ...
  for (const dir of dirs) {
    ...
    for (const ent of ents) {
      ...
      if (ent.name === 'harness.yaml') {
        // project active harness pointer — skip as registry entry
        continue;
      }
      try {
...
/** Resolve harness for a session; unknown/missing → spockify with warning. */
export function resolveHarnessOrFallback(
  id: string | undefined,
  projectRoot?: string,
): { harness: HarnessYaml; fallbackReason?: string } {
  const want = id || readActiveHarnessId(projectRoot);
  if (want === 'spockify') {
    return { harness: { ...DEFAULT_SPOCKIFY } };
  }
  ...
  // Presence check for command on PATH / absolute
  const cmd = found.command;
  const abs = cmd.startsWith('/') || /^[A-Za-z]:\\/.test(cmd);
  if (abs && !fs.existsSync(cmd)) {
    return {
      harness: { ...DEFAULT_SPOCKIFY },
      fallbackReason: `harness ${found.id}: command not found (${cmd}); using spockify`,
    };
  }
