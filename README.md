...
Self-hosted coding stack for open-weight models. One kernel. Your GPUs.

Chat, IDE, and CLI on LiteLLM. Default harness is Spockify. Wichy-class tool loops and OpenHands-class SWE sandboxes are optional backends — not the product.

Quality is model × harness × slice. More GPUs add chat replicas and eval sessions. They do not raise pass@1.

## Pools

Three pools. SWE and canary never share the chat ingress.

| Pool | Runs | Hard rule |
| --- | --- | --- |
| Chat | Chat UI, router, small and mid chat tags | No SWE, no multi-day benches |
| Coding sessions | One `runHarness` process per session | One worker slot per session |
| Eval | Canary, Lite slices, plugin tests | Never the chat ingress |

Workstation (Compose, or the IDE plus your API) is the first way to run this. It is not the product name.

## Site

Live: apex splash at [`/`](https://spockify.eu/), chat login at [`/chat`](https://spockify.eu/chat), IDE at [`/ide`](https://spockify.eu/ide). Source: [`site/`](site/) (published via `k8s/ide-site/www/`). Details: [`docs/SITE.md`](docs/SITE.md).

## Get Started

Local compose, desktop IDE, and CLI. Workstation first.
...
Details: [docker/ide/README.md](docker/ide/README.md). Linux packaging: `make release`.
...
Compose above is the workstation path. Do not point SWE or canary at this chat stack.
